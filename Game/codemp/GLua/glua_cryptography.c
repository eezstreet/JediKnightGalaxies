...
static int GLua_Cryptography_CreateDigestContext(lua_State *L) {
	...
	ctx = lua_newuserdata(L, sizeof(GLua_EVP_MD_CTX_t));
...
static int GLua_Cryptography_CreateCipherContext(lua_State *L) {
	...
	ctx = lua_newuserdata(L, sizeof(GLua_EVP_CIPHER_CTX_t));
...
static int GLua_Cryptography_Digest(lua_State *L) {
	...
	lua_pushlstring(L, digest, digestlen);
	return 1;
}
...
static int GLua_Cryptography_Encrypt(lua_State *L) {
	...
	EVP_EncryptUpdate(&ctx, encrypted, &tmplen, data, length);
	enclen += tmplen;

	EVP_EncryptFinal_ex(&ctx, encrypted + enclen, &tmplen);
	enclen += tmplen;

	EVP_CIPHER_CTX_cleanup(&ctx);

	lua_pushlstring(L, encrypted, enclen);
	free((void *)encrypted);
	return 1;
}
...
static int GLua_Cryptography_Decrypt(lua_State *L) {
	...
	EVP_DecryptUpdate(&ctx, decrypted, &tmplen, data, length);
	declen += tmplen;

	if (!EVP_DecryptFinal_ex(&ctx, decrypted + declen, &tmplen)) {
		EVP_CIPHER_CTX_cleanup(&ctx);
		free((void *)decrypted);
		return 0;
	}
	...
	lua_pushlstring(L, decrypted, declen);
	free((void *)decrypted);
	return 1;
}
