...
static int GLua_Entity_SetTarget(lua_State *L) {
	gentity_t *ent = GLua_CheckEntity(L, 1);
	if (!ent) return 0;
	G_NewString2(&ent->target, luaL_checkstring(L,2));
	return 0;
}
...
static int GLua_Entity_SetTargetName(lua_State *L) {
	gentity_t *ent = GLua_CheckEntity(L, 1);
	if (!ent) return 0;
	G_NewString2(&ent->targetname, luaL_checkstring(L,2));
	return 0;
}
