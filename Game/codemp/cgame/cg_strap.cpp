...
{
	return cgi.G2API_GetBoltMatrix(ghoul2, modelIndex, boltIndex, matrix, angles, position, frameNum, modelList, scale);
}

qboolean strap_G2API_GetBoltMatrix_NoReconstruct(void *ghoul2, const int modelIndex, const int boltIndex, mdxaBone_t *matrix,
								const vec3_t angles, const vec3_t position, const int frameNum, qhandle_t *modelList, vec3_t scale)
{
	return cgi.G2API_GetBoltMatrix_NoReconstruct(ghoul2, modelIndex, boltIndex, matrix, angles, position, frameNum, modelList, scale);
}

qboolean strap_G2API_GetBoltMatrix_NoRecNoRot(void *ghoul2, const int modelIndex, const int boltIndex, mdxaBone_t *matrix,
								const vec3_t angles, const vec3_t position, const int frameNum, qhandle_t *modelList, vec3_t scale)
{
	return cgi.G2API_GetBoltMatrix_NoRecNoRot(ghoul2, modelIndex, boltIndex, matrix, angles, position, frameNum, modelList, scale);
}
...
{
	return cgi.G2API_SetBoneAngles(ghoul2, modelIndex, boneName, angles, flags, up, right, forward, modelList, blendTime, currentTime);
}
...
{
	return cgi.G2API_SetBoneAnim(ghoul2, modelIndex, boneName, startFrame, endFrame, flags, animSpeed, currentTime, setFrame, blendTime);
}
...
{
	return cgi.G2API_GetBoneAnim(ghoul2, boneName, currentTime, currentFrame, startFrame, endFrame, flags, animSpeed, modelList, modelIndex);
}

void strap_G2API_SetRagDoll(void *ghoul2, sharedRagDollParams_t *params)
{
	cgi.G2API_SetRagDoll(ghoul2, params);
}

void strap_G2API_AnimateG2Models(void *ghoul2, int time, sharedRagDollUpdateParams_t *params)
{
	cgi.G2API_AnimateG2Models(ghoul2, time, params);
}
...
{
	return cgi.G2API_SetBoneIKState(ghoul2, time, boneName, ikState, params);
}

qboolean strap_G2API_IKMove(void *ghoul2, int time, sharedIKMoveParams_t *params)
{
	return cgi.G2API_IKMove(ghoul2, time, params);
}

int strap_FS_FOpenFile( const char *qpath, fileHandle_t *f, fsMode_t mode )
{
    return cgi.FS_FOpenFile (qpath, f, mode);
}
...
{
    return cgi.FS_GetFileList (path, extension, listbuf, bufsize);
}
