...
// REQUIRED: DirectX SDK, dxguid.lib and dinput8.lib includes
...
void XboxMovement(usercmd_t *cmd)
{
	...
	if( in_controlScheme.integer == 0 )
	{
		...
		if( cg.time > lastBButton )
		{
			...
			{
				cg.crouchToggled = (qboolean)!cg.crouchToggled;
...
// Requires MotionInJoy drivers for full effect
...
void Dualshock_Init(qboolean sixaxis)
{
	// Do a check to make sure that we can access the joystick
	if((wNumDevs = joyGetNumDevs()) == 0) 
		return;
	qboolean bDev1Attached = (qboolean)(joyGetPos(JOYSTICKID1,&ps3info) != JOYERR_UNPLUGGED);
	...
	joySetCapture((HWND)0xB8F038, JOYSTICKID1, NULL, FALSE);
}

void Dualshock_Update(qboolean sixaxis)
{
	// Do a check to make sure that we can access the joystick
	if((wNumDevs = joyGetNumDevs()) == 0) 
		return;
...
void Dualshock_Movement(usercmd_t *cmd, qboolean sixaxis)
{
}
#endif // __PS3CONTROLLER_DUALSHOCK
// ===========================================================================================================================
...
#ifdef __PS3CONTROLLER_DUALSHOCK
			case JOY_DUALSHOCK:
				Dualshock_Init(qfalse);
				break;
			case JOY_SIXAXIS:
				Dualshock_Init(qtrue);
				break;
#endif // __PS3CONTROLLER_DUALSHOCK
#ifdef __WINGMANJOYSTICK
			case JOY_WINGMAN:
...
#ifdef __PS3CONTROLLER_DUALSHOCK
			case JOY_DUALSHOCK:
				Dualshock_Update(qfalse);
				break;
			case JOY_SIXAXIS:
				Dualshock_Update(qtrue);
				break;
#endif // __PS3CONTROLLER_DUALSHOCK
#ifdef __WINGMANJOYSTICK
			case JOY_WINGMAN:
...
#ifdef __PS3CONTROLLER_DUALSHOCK
			case JOY_DUALSHOCK:
				Dualshock_Movement(cmd, qfalse);
				break;
			case JOY_SIXAXIS:
				Dualshock_Movement(cmd, qtrue);
				break;
#endif // __PS3CONTROLLER_DUALSHOCK
#ifdef __WINGMANJOYSTICK
			case JOY_WINGMAN:
...
void JKG_DoDebugController(void)
{
	cvar_t *in_joystick = Cvar_Get("in_joystick", "0", 0);
#ifdef __SIXAXIS_DEBUG
	if(in_joystick->integer == JOY_DUALSHOCK ||
		in_joystick->integer == JOY_SIXAXIS)
	{
		qboolean register sixaxis = static_cast<qboolean>(in_joystick->integer == JOY_SIXAXIS);
		Dualshock_DisplayDebugData(sixaxis);
	}
#endif // __SIXAXIS_DEBUG
}
}