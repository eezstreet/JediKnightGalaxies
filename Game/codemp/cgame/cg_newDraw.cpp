...
static void CG_Text_Paint_Limit(float *maxX, float x, float y, float scale, vec4_t color, const char* text, float adjust, int limit, int iMenuFont) 
{
	...
	//float fMax = *maxX;
	int iPixelLen = cgi.R_Font_StrLenPixels(text, iFontIndex, scale);
	if (x + iPixelLen > *maxX)
	{
		...
		while (*psText && (x + cgi.R_Font_StrLenPixels(sTemp, iFontIndex, scale)<=*maxX) 
				...
				)
		{
			...
			uiLetter = cgi.AnyLanguage_ReadCharFromString(psText, &iAdvanceCount, &bIsTrailingPunctuation);
		...
		if ( ci->infoValid && ci->team == cg.snap->ps.persistant[PERS_TEAM]) {
			...
			for (j = 0; j <= PW_NUM_POWERUPS; j++) {
				if (ci->powerups & (1 << j)) {
					...
					if (item) {
						CG_DrawPic( xx, y, PIC_WIDTH, PIC_WIDTH, cgi.R_RegisterShader( item->icon ) );
						xx += PIC_WIDTH;
					}
				}
			}
...
void CG_KeyEvent(int key, qboolean down) {
  ...
  //if (key == cgi.Key_GetKey("teamMenu") || !Display_CaptureItem(cgs.cursorX, cgs.cursorY)) {
