...
static void CG_General( centity_t *cent ) {
	...
	switch ( cent->currentState.weapon )
	{
	    case WP_TRIP_MINE:
	    case WP_DET_PACK:
	        JKG_RenderExplosive (cent, (qboolean)(cent->currentState.eFlags & EF_ALT_FIRING));
	        return;
...
*/
	...
	/*if ( item->giType == IT_WEAPON && (cent->currentState.eFlags & EF_DROPPEDWEAPON) )
	{
	    ent.hModel = 0;
	    ent.ghoul2 = CG_G2WeaponInstance (cent, item->giTag, cent->currentState.weaponVariation);
	}*/
...
//	int	col;
	...
	if ( 0 )
	{
		...
		if ( s1->apos.trBase[0] && iLen >= s1->apos.trBase[0] )
		{
			...
			else
			{
				...
				JKG_RenderProjectileDeath (cent, org, fwd, (qboolean)(s1->eFlags & EF_ALT_FIRING));
    ...
    if ( s1->weapon != WP_SABER )
    {
        JKG_RenderProjectile (cent, (qboolean)(s1->eFlags & EF_ALT_FIRING));
        return;
    }
#endif
