...
static void CG_DrawAmmo( centity_t	*cent,menuDef_t *menuHUD)
{
	...
	if ( GetWeaponData( cent->currentState.weapon, cent->currentState.weaponVariation )->primary.cost == 0 && GetWeaponData( cent->currentState.weapon, cent->currentState.weaponVariation )->secondary.cost == 0)
	{ //just draw "infinite"
		text = "Ammo: Infinite";
	}
	else
	...
	if (weaponData[cent->currentState.weapon].useClip) {
	...
	else
	{
		...
		if (focusItem)
		{
			...
			{
				if (weaponData[cent->currentState.weapon].useClip) {
					inc = (float) weaponData[cent->currentState.weapon].clipSize / MAX_HUD_TICS;
				} else {
			...
			if (weaponData[cent->currentState.weapon].useClip) {
				// Display the amount of clips
				// TODO: do this properly xD
				float temp;
				temp = ceil((float)ps->ammo[weaponData[cent->currentState.weapon].ammoIndex] / (float)weaponData[cent->currentState.weapon].clipSize);
				trap_R_Font_DrawString(focusItem->window.rect.x, focusItem->window.rect.y - 150, va("Clips: %i", (int)temp), hudTintColor, cgDC.Assets.qhSmallFont, -1, 0.8f);
			}
		}
	}
...
void MiniMap_Render(menuDef_t *menu, float radius);
...
void CG_DrawHUD(centity_t	*cent)
{
	...
	if (cg_hudFiles.integer)
	{
		...
		{
			Com_sprintf(ammoString, sizeof(ammoString), "%i", cg.snap->ps.ammo[GetWeaponAmmoIndex( cent->currentState.weapon, cent->currentState.weaponVariation )]);
		}
	...
	*/
	...
	if (menuHUD)
	{
		...
		{
			temp = va("%i", 0);
			x = ((focusItem->window.rect.w/2) - (trap_R_Font_StrLenPixels(temp, MenuFontToHandle(1), 0.6f) / 2)) + focusItem->window.rect.x;
		...
		if (focusItem)
		{
			temp = va("%s", Info_ValueForKey(CG_ConfigString(CS_PLAYERS + cg.snap->ps.clientNum), "n"));
			//CG_DrawRect(focusItem->window.rect.x,focusItem->window.rect.y,focusItem->window.rect.w,focusItem->window.rect.h,1,colorWhite);
			// To avoid rounding errors, we get the length at scale 1, then adjust it
			x = ((focusItem->window.rect.w/2) - (((float)trap_R_Font_StrLenPixels(temp, MenuFontToHandle(1), 1) * 0.5f) / 2)) + focusItem->window.rect.x;
			//CG_DrawRect(focusItem->window.rect.x, focusItem->window.rect.y, focusItem->window.rect.w, focusItem->window.rect.h, 1, colorWhite);
	...
	/*  Jedi Knight Galaxies -- Dont draw the score on screen
...
static float CG_DrawFPS( float y ) {
	...
	trap_R_Font_DrawString(480 - w, y+25, s, fpscolor, cgDC.Assets.qhSmallFont | STYLE_DROPSHADOW, -1, 0.5f);
	//CG_DrawBigString( 635 - w + xOffset, y + 2, s, 1.0F);

	return y; //y + BIGCHAR_HEIGHT + 4;
}
...
/*
	...
	seconds -= tens * 10;
*/
	s = va( "%.2i:%.2i:%.2i", hours, mins, seconds );
...
void CG_DrawSiegeInfo(centity_t *cent, float chX, float chY, float chW, float chH)
{
	...
	//now draw his ammo
	ammoMax = GetWeaponAmmoMax( cent->currentState.weapon, cent->currentState.weaponVariation );
	...
	if ( !GetWeaponData( cent->currentState.weapon, cent->currentState.weaponVariation )->primary.cost && !GetWeaponData( cent->currentState.weapon, cent->currentState.weaponVariation )->secondary.cost )
...
static void CG_ScanForCrosshairEntity( void ) {
	...
	{ //then do a trace with ghoul2 models in mind
		...
		//[USE_ITEMS]
			...
			ignore, CONTENTS_SOLID|CONTENTS_BODY|CONTENTS_ITEM );
        //[/USE_ITEMS]
	...
	if (cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR)
	{
		if (trace.entityNum < /*MAX_CLIENTS*/ENTITYNUM_WORLD)
		{
            ...
            //[USE_ITEMS]
			if (cg.crosshairClientNum < ENTITYNUM_WORLD && !(trace.contents & CONTENTS_ITEM))
			//[/USE_ITEMS]
			{
...
static void CG_DrawCrosshairNames( void ) {
    ...
    {
        UI_DrawProportionalString (320, 170, "Press E to pick up weapon", UI_CENTER, colorTable[CT_WHITE]);
        return;
    }
    else
    //[/USE_ITEMS]
...
====================================*/
#define	CHATBOX_CUTOFF_LEN	225 //550
...
void CG_ChatBox_AddString(char *chatStr, int fadeLevel)
{
	...
	strcpy(chat->string, va("^7%s %s", CG_ChatBox_TimeStamp()  ,ChatBox_UnescapeChat(chatStr)));
	...
	chatLen = trap_R_Font_StrLenPixels(chat->string, cgDC.Assets.qhSmallFont, 1); //CG_Text_Width(chat->string, 1.0f, FONT_SMALL);
	chatLen *= 0.4f;
	...
	{ //we have to break it into segments...
		...
		while (chat->string[i])
		{
			...
			s[0] = chat->string[i];
			s[1] = 0;
			chatLen += ((float)trap_R_Font_StrLenPixels(s, cgDC.Assets.qhSmallFont, 1) * 0.4f); //CG_Text_Width(s, 0.65f, FONT_SMALL);
	...
	if (item)
	{
		...
		for (i=0; i<12; i++) {
			...
			if (cg.chatItems[idx].active) {
				...
				trap_R_Font_DrawString(
					item->window.rect.x,
					item->window.rect.y + (line*8),
					cg.chatItems[idx].string,
					color,
					cgDC.Assets.qhSmallFont | STYLE_DROPSHADOW,
					-1,
					0.4f);
