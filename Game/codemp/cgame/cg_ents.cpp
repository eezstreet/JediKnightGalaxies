...
static void CG_EntityEffects( centity_t *cent ) {
	...
	// constant light glow
	if ( cent->currentState.constantLight && !(cent->currentState.eType == ET_PLAYER || cent->currentState.eType == ET_NPC)) { // FIX: Dont do dynamic light on player/NPC entities, as this value is used for weapon charge
		int		cl;
		int		i, r, g, b;
...
void FX_DrawPortableShield(centity_t *cent)
{
	...
	cgi.Cvar_VariableStringBuffer("cl_paused", buf, sizeof(buf));
...
void CG_Disintegration(centity_t *cent, refEntity_t *ent)
{
	...
	if ( cg.time - ent->endTime < 1000 && (cg_timescale.value * cg_timescale.value * random()) > 0.05f )
	{
		...
		fxOrg[2] += crandom() * 20;
		cgi.FX_PlayEffectID( cgs.effects.mDisruptorDeathSmoke, fxOrg, fxDir, -1, -1 );
...
*/
	...
	{ //special case for client limbs
		...
		if (!cent->ghoul2)
		{
			...
			{
				if(!clEnt->armorGhoul2[i] || !cgi.G2_HaveWeGhoul2Models(clEnt->armorGhoul2[i]))
				{
					continue;
				}
				if (clEnt->localAnimIndex < NUM_RESERVED_ANIMSETS)
				{ //humanoid
					cgi.G2API_SetBoneAngles(clEnt->armorGhoul2[i], 0, "model_root", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 100, cg.time);
					cgi.G2API_SetBoneAngles(clEnt->armorGhoul2[i], 0, "pelvis", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0, cg.time); 
					cgi.G2API_SetBoneAngles(clEnt->armorGhoul2[i], 0, "thoracic", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0, cg.time); 
					cgi.G2API_SetBoneAngles(clEnt->armorGhoul2[i], 0, "upper_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 100, cg.time);
					cgi.G2API_SetBoneAngles(clEnt->armorGhoul2[i], 0, "lower_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 100, cg.time);
					cgi.G2API_SetBoneAngles(clEnt->armorGhoul2[i], 0, "cranium", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, POSITIVE_X, cgs.gameModels, 100, cg.time);
				}
				else
				{
					cgi.G2API_SetBoneAngles(clEnt->armorGhoul2[i], 0, "model_root", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 100, cg.time);
				...
				{
					cgi.G2API_DuplicateGhoul2Instance(clEnt->armorGhoul2[i], &cent->armorGhoul2[i]);
				}
			}
			...
			if ( newBolt != -1 )
			{
				...
				//cgi.FX_PlayEffectID(cgs.effects.mBlasterSmoke, boltOrg, boltAng, -1, -1);
			}
	...
	{ //bodies should have a radius as well
		...
		for(i = 0; i < ARMSLOT_MAX; i++)
		{
			if(cent->armorGhoul2[i] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[i]))
			{
				cent->lerpAngles[PITCH] = 0;
				cent->lerpAngles[ROLL] = 0;
				cgi.G2API_SetBoneAngles(cent->armorGhoul2[i], 0, "pelvis", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0, cg.time); 
				cgi.G2API_SetBoneAngles(cent->armorGhoul2[i], 0, "thoracic", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0, cg.time); 
				cgi.G2API_SetBoneAngles(cent->armorGhoul2[i], 0, "upper_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 100, cg.time);
				cgi.G2API_SetBoneAngles(cent->armorGhoul2[i], 0, "lower_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 100, cg.time);
				cgi.G2API_SetBoneAngles(cent->armorGhoul2[i], 0, "cranium", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, POSITIVE_X, cgs.gameModels, 100, cg.time);
			}
		}
	}
		...
		if (cent->currentState.owner != cg.snap->ps.clientNum && !TeamFriendly( cent->currentState.owner ) && !(cg.snap->ps.fd.forcePowersActive & (1 << FP_SEE))) {
			...
			if ((cg.time >> 10 & 1) && TeamFriendly( cent->currentState.owner ) && cent->currentState.owner != cg.snap->ps.clientNum) {
				...
				if (cent->currentState.bolt2 == 1)
				{
					...
					cgi.FX_PlayEffectID( beamID, beamOrg, cent->currentState.pos.trDelta, -1, -1 );
				}
				else
				{
					int i = 0;
...
*/
		...
		!cg_simpleItems.integer)
	{
		...
		if (!doGrey)
		{
			cgi.FX_PlayEffectID(cgs.effects.itemCone, ent.origin, uNorm, -1, -1);
		}
	}
...
//	int	col;
	...
	{//using an over-ridden trail effect!
		...
		{ //a vehicle with an override for the weapon trail fx or model
			cgi.FX_PlayEffectID( g_vehWeaponInfo[s1->otherEntityNum2].iShotFX, cent->lerpOrigin, forward, -1, -1 );
		...
		{//a regular missile
			cgi.FX_PlayEffectID( cgs.gameEffects[s1->otherEntityNum2], cent->lerpOrigin, forward, -1, -1 );
			if ( s1->loopSound )
			{
				vec3_t	velocity;
				BG_EvaluateTrajectoryDelta( &cent->currentState.pos, cg.time, velocity );
...
*/
	...
	{//special code for adding the beam to the attached tripwire mine
		...
		cgi.FX_PlayEffectID( cgs.effects.mTripMineLaser, beamOrg, ent.axis[0], -1, -1 );
	}
}
...
//	}
	...
	{
		if (VectorDistance(cent->lerpOrigin, cg_entities[cg.clientNum].lerpOrigin) <= cent->currentState.otherEntityNum2)
		{
	...
	{
		if ( capture_ent_num != 0 && capture_ent_num != cent->currentState.number
			&& VectorDistance(capturingflagent->lerpOrigin, cg_entities[cg.clientNum].lerpOrigin) <= cg_entities[capture_ent_num].currentState.otherEntityNum2)
	...
	{// Register basic flag model ready for use...
		...
		cgs.media.redFlagRadarShader = cgi.R_RegisterShader("gfx/radar/red_flag");
		cgs.media.blueFlagRadarShader = cgi.R_RegisterShader("gfx/radar/blue_flag");
		cgs.media.neutralFlagRadarShader = cgi.R_RegisterShader("gfx/radar/neutral_flag"); // Warzone...
	}
...
void CG_Ammo_Crate( centity_t *cent ) {
	...
	if (!cent->currentState.modelindex2)
		cent->currentState.modelindex2 = cgi.R_RegisterModel( "models/doa/doa_cabinet/ammo_close.md3" );

	if (!cent->currentState.modelindex)
		cent->currentState.modelindex = cgi.R_RegisterModel( "models/doa/doa_cabinet/ammo_open.md3" );
...
void CG_Health_Crate( centity_t *cent ) {
	...
	if (!cent->currentState.modelindex2)
		cent->currentState.modelindex2 = cgi.R_RegisterModel( "models/doa/doa_cabinet/health_close.md3" );

	if (!cent->currentState.modelindex)
		cent->currentState.modelindex = cgi.R_RegisterModel( "models/doa/doa_cabinet/health_open.md3" );
...
void CG_CalcEntityLerpPositions( centity_t *cent ) {
	...
	if ( !cg_smoothClients.integer ) {
		...
		if ( cent->currentState.number < MAX_CLIENTS || cent->currentState.eType == ET_NPC ) { // UQ1: NPCs too pls!!!
	...
	if ( cent->interpolate && cent->currentState.pos.trType == TR_LINEAR_STOP 
		&& (cent->currentState.number < MAX_CLIENTS || cent->currentState.eType == ET_NPC)) { // UQ1: NPCs too pls!!!
		CG_InterpolateEntityPosition( cent );
		goAway = qtrue;
	}
...
static void CG_FX( centity_t *cent ) 
{
	...
	if (efxIndex)
	{
		if (s1->isPortalEnt)
		{
			cgi.FX_PlayPortalEffectID(efxIndex, cent->lerpOrigin, fxDir, -1, -1 );
		}
		else
		{
			cgi.FX_PlayEffectID(efxIndex, cent->lerpOrigin, fxDir, -1, -1 );
		}
	}
...
defaultoffsetposition:
		...
		if (objectID)
		{
			...
			cgi.FX_PlayEffectID(objectID, useOrigin, useAngles, -1, -1);
		}
	}
