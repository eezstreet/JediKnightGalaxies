...
//	float	len;
	...
	cgi.FX_AddLine( start, end, 0.1f, 6.0f, 0.0f, 
							1.0f, 0.0f, 0.0f,
							WHITE, WHITE, 0.0f,
...
void FX_DisruptorAltShot( vec3_t start, vec3_t end, qboolean fullCharge )
{
	cgi.FX_AddLine( start, end, 0.1f, 10.0f, 0.0f, 
							1.0f, 0.0f, 0.0f,
							WHITE, WHITE, 0.0f,
							175, cgi.R_RegisterShader( "gfx/effects/redLine" ), 
							FX_SIZE_LINEAR | FX_ALPHA_LINEAR );
...
void FX_DisruptorAltMiss( vec3_t origin, vec3_t normal )
{
	...
	WHITE, WHITE, 0.0f, 4000, cgi.R_RegisterShader( "gfx/effects/smokeTrail" ), FX_ALPHA_WAVE );
	*/
	...
	b.rgbParm = 0.0f;
	b.killTime = 4000;
	b.shader = cgi.R_RegisterShader( "gfx/effects/smokeTrail" );
	b.flags = FX_ALPHA_WAVE;

	cgi.FX_AddBezier(&b);

	cgi.FX_PlayEffectID( cgs.effects.disruptorAltMissEffect, origin, normal, -1, -1 );
}
...
{
	cgi.FX_PlayEffectID( cgs.effects.disruptorAltHitEffect, origin, normal, -1, -1 );
}
...
{
	cgi.FX_PlayEffectID( cgs.effects.disruptorWallImpactEffect, origin, normal, -1, -1 );
}
...
{
	cgi.FX_PlayEffectID( cgs.effects.disruptorFleshImpactEffect, origin, normal, -1, -1 );
}
