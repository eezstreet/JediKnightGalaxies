...
void FX_BryarProjectileThink(  centity_t *cent, const struct weaponInfo_s *weapon )
{
	...
	cgi.FX_PlayEffectID( cgs.effects.bryarShotEffect, cent->lerpOrigin, forward, -1, -1 );
}
...
{
	cgi.FX_PlayEffectID( cgs.effects.bryarWallImpactEffect, origin, normal, -1, -1 );
}
...
void FX_BryarHitPlayer( vec3_t origin, vec3_t normal, qboolean humanoid )
{
	if ( humanoid )
	{
		cgi.FX_PlayEffectID( cgs.effects.bryarFleshImpactEffect, origin, normal, -1, -1 );
	}
	else
	{
		cgi.FX_PlayEffectID( cgs.effects.bryarDroidImpactEffect, origin, normal, -1, -1 );
	}
}
...
void FX_BryarAltProjectileThink(  centity_t *cent, const struct weaponInfo_s *weapon )
{
	...
	for (t = 1; t < cent->currentState.generic1; t++ )
	{
		...
		cgi.FX_PlayEffectID( cgs.effects.bryarPowerupShotEffect, cent->lerpOrigin, forward, -1, -1 );
	}
...
void FX_BryarAltHitWall( vec3_t origin, vec3_t normal, int power )
{
	...
	case 4:
	case 5:
		cgi.FX_PlayEffectID( cgs.effects.bryarWallImpactEffect3, origin, normal, -1, -1 );
		break;

	case 2:
	case 3:
		cgi.FX_PlayEffectID( cgs.effects.bryarWallImpactEffect2, origin, normal, -1, -1 );
		break;
...
void FX_TurretProjectileThink(  centity_t *cent, const struct weaponInfo_s *weapon )
{
	...
	cgi.FX_PlayEffectID( cgs.effects.turretShotEffect, cent->lerpOrigin, forward, -1, -1 );
}
...
{
	cgi.FX_PlayEffectID( cgs.effects.concussionImpactEffect, origin, normal, -1, -1 );
}
...
void FX_ConcussionProjectileThink(  centity_t *cent, const struct weaponInfo_s *weapon )
{
	...
	cgi.FX_PlayEffectID( cgs.effects.concussionShotEffect, cent->lerpOrigin, forward, -1, -1 );
}
...
{
	//"concussion/beam"
	cgi.FX_AddLine( start, end, 0.1f, 10.0f, 0.0f, 
							1.0f, 0.0f, 0.0f,
							WHITE, WHITE, 0.0f,
							175, cgi.R_RegisterShader( "gfx/effects/blueLine" ), 
							FX_SIZE_LINEAR | FX_ALPHA_LINEAR );

	// add some beef
	cgi.FX_AddLine( start, end, 0.1f, 7.0f, 0.0f, 
						1.0f, 0.0f, 0.0f,
						BRIGHT, BRIGHT, 0.0f,
						150, cgi.R_RegisterShader( "gfx/misc/whiteline2" ), 
						FX_SIZE_LINEAR | FX_ALPHA_LINEAR );
}
