...
sfxHandle_t	CG_CustomSound( int clientNum, const char *soundName ) {
	...
	for ( i = 0 ; i < MAX_CUSTOM_SOUNDS ; i++ )
	{
		...
		else if ( i < numCSiegeSounds && (cgs.gametype >= GT_TEAM || cg_buildScript.integer) && i < numCSiegeSounds && !strcmp( lSoundName, bg_customSiegeSoundNames[i] ) )
		{ //siege only
...
qboolean CG_ParseSurfsFile( const char *modelName, const char *skinName, char *surfOff, char *surfOn ) 
{
	...
	// load the file
	len = cgi.FS_FOpenFile( sfilename, &f, FS_READ );
	if ( len <= 0 ) 
	{//no file
		return qfalse;
	}
...
/*
==========================
CG_RegisterClientArmorModelname

Code: ArmorSys
Original Author: redsaurus
Modified by eezstreet for JKG's armor system
==========================
*/
extern cgArmorData_t armorMasterTable[MAX_ARMOR_PIECES];
extern void JKG_CG_Armor_GetPartFromSurfString(int token, const char *string, char *buffer);
extern int JKG_G2_GetNumberOfSurfaces ( const char *modelPath );
qboolean CG_RegisterClientArmorModelname( centity_t *cent, int armorNum, int clientNum, int slot) {
	...
	// eez -- Variables for modelName and skinName
	const char *modelName = armorMasterTable[armorNum].model;
	const char *skinName = armorMasterTable[armorNum].skin;
	...
	if (cent->armorGhoul2 && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[slot]))
	{
	...
	if (!checkSkin)
	{
		checkSkin = cgi.R_RegisterSkin(va("models/players/%s/model_default.skin", modelName, skinName));
	}
	Com_sprintf( afilename, sizeof( afilename ), "models/players/%s/model.glm", modelName );
	handle = cgi.G2API_InitGhoul2Model(&cent->armorGhoul2[slot], afilename, 0, checkSkin, 0, 0, 0);
	...
	armorGhoul2 = cent->armorGhoul2[slot];

	// rsus -- The model is now loaded.

	cgi.G2API_SetSkin(cent->armorGhoul2[slot], 0, checkSkin, checkSkin);

	if ( CG_ParseSurfsFile( modelName, skinName, surfOff, surfOn ) )
	{// rsus -- turn on/off any surfs
		const char   *token;
		const char   *p;
		...
		if ( surfOff && surfOff[0] )
		{
			...
			while ( 1 )
			{
				token = COM_ParseExt( &p, qtrue );
				if ( !token || !token[0] )
				{//rsus -- reached end of list
					break;
				}
				//rsus -- turn off this surf
				cgi.G2API_SetSurfaceOnOff( armorGhoul2, token, 0x00000002/*G2SURFACEFLAG_OFF*/ );
			}
		}
		if ( surfOn && surfOn[0] )
		{
			...
			while ( 1 )
			{
				...
				//rsus -- turn on this surf
				cgi.G2API_SetSurfaceOnOff( armorGhoul2, token, 0 );
			}
		}
	}

	//FIXME: BAD! This is using the wrong field!
    JKG_CG_ShowOnlySelectedSurfaces (armorGhoul2, afilename, armorMasterTable[armorNum].surfOnThisString);
...
void CG_LoadCISounds(clientInfo_t *ci, qboolean modelloaded, int clientNum)
{
	...
	{//try default sounds.cfg first
		fLen = cgi.FS_FOpenFile(va("models/players/%s/sounds.cfg", dir), &f, FS_READ);
		if ( !f ) 
		{//no?  Look for _default sounds.cfg
			fLen = cgi.FS_FOpenFile(va("models/players/%s/sounds_default.cfg", dir), &f, FS_READ);
		}
	}
	else
	{//use the .skin associated with this skin
		fLen = cgi.FS_FOpenFile(va("models/players/%s/sounds_%s.cfg", dir, ci->skinName), &f, FS_READ);
		if ( !f ) 
...
void CG_LoadClientInfo( clientInfo_t *ci ) {
	...
	*/
	...
	if (clientNum != -1 && ci->ghoul2Model && cgi.G2_HaveWeGhoul2Models(ci->ghoul2Model))
	{
		...
		//eezstreet - Armor surf off code
		//Surf off the lower parts.
		{
			char surfOff[1024];
			int i = 0, ijk;
			for(ijk = 0; ijk < ARMSLOT_MAX; ijk++)
			{
				...
				while ( 1 )
				{
					if(armorMasterTable[cg_entities[clientNum].equippedArmor[ijk]].surfOffLowerString)
					{
						...
						cgi.G2API_SetSurfaceOnOff( cg_entities[clientNum].ghoul2, surfOff, 0x00000002/*G2SURFACEFLAG_OFF*/ );
						i++;
					}
					else
					{
				...
				while ( 1 )
				{
					if(armorMasterTable[cg_entities[clientNum].equippedArmor[ijk]].surfOffThisString)
					{
						...
						if(!surfOff || !surfOff[0] || surfOff[0] == ',')
							break; //Piece isn't valid
						cgi.G2API_SetSurfaceOnOff( cg_entities[clientNum].armorGhoul2, surfOff, 0x00000002/*G2SURFACEFLAG_OFF*/ );
						i++;
					}
					else
					{
	...
	*ci = newInfo;
	...
	{	// Copy the new ghoul2 model to the centity.
		...
		if (anim)
		{
			...
			{
				if(cent->armorGhoul2[i] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[i]))
				{
					cgi.G2API_SetBoneAnim(cent->armorGhoul2[i], 0, "model_root", firstFrame, anim->firstFrame + anim->numFrames, flags, animSpeed, cg.time, setFrame, 150);
				}
			}
		...
		if (anim)
		{
			...
			for(i = 0; i < ARMSLOT_MAX; i++)
			{
				if(cent->armorGhoul2[i] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[i]))
				{
					cgi.G2API_SetBoneAnim(cent->armorGhoul2[i], 0, "lower_lumbar", firstFrame, anim->firstFrame + anim->numFrames, flags, animSpeed, cg.time, setFrame, 150);
				}
			}
		...
		//eezstreet - Armor surf off code
		{
			...
			for(ijk = 0; ijk < ARMSLOT_MAX; ijk++)
			{
				cgArmorData_t armor = armorMasterTable[cg_entities[clientNum].equippedArmor[ijk]];
								...
								centity_t *const cent, footstepType_t footStepType ) 
{
	...
	cgi.CM_BoxTrace( &trace, origin, end, mins, maxs, 0, MASK_PLAYERSOLID );
	...
	if ( effectID != -1 )
	{
		cgi.FX_PlayEffectID( effectID, trace.endpos, trace.plane.normal, -1, -1 );
	}
...
void CG_PlayerAnimEventDo( centity_t *cent, animevent_t *animEvent )
{
	...
	case AEV_EFFECT:
		...
		if ( animEvent->eventData[AED_BOLTINDEX] != -1 )
		{
			...
			cgi.FX_PlayEffectID(animEvent->eventData[AED_EFFECTINDEX], bPoint, bAngle, -1, -1);
		}
		else
		{
			vec3_t bAngle;
			...
			VectorSet(bAngle, 0, 1, 0);
			cgi.FX_PlayEffectID(animEvent->eventData[AED_EFFECTINDEX], cent->lerpOrigin, bAngle, -1, -1);
		}
#endif
		break;
...
// Draw the flashlight for this player (if its on)
void CG_DoFieldLight( centity_t *cent ) {
	vec3_t muzzle;
	vec3_t forward, right, up;
	vec3_t end;

	
	/*if (!(cent->playerState->eFlags & EF_FIELDLIGHT)) */{
		return;		// If the flashlight ain't on, ignore
	}
	//if (cent->playerState->weapon <= WP_SABER) {
	//	return;		// Sabers, melee and unarmed people has no fieldlight
	//}
	CG_CalcMuzzlePoint( cent->currentState.number, muzzle );
	AngleVectors(cent->playerState->viewangles, forward, right, up);
	VectorMA (muzzle, 5, forward, end);

	//Trace Position
	/*
	CG_Trace (&tr, muzzle, NULL, NULL, end, cent->currentState.number, MASK_SHOT );
	if (tr.fraction == 1.0f) {
		return;		// We didn't hit anything
	}
	// Get the distance we reached
	VectorSubtract(muzzle, tr.endpos, delta);
	distance = VectorLength(delta);

	VectorMA (tr.endpos, 50, tr.plane.normal, lightorg);
	if (distance > 512) {
		intensity = (512.0f - (distance-512.0f)) / 2.0f;
	} else {
		intensity = (distance) / 2.0f;
		if (intensity < 50) {
			intensity = 50;
		}
	}
	cgi.R_AddLightToScene(lightorg, intensity, 1, 1, 1);
	*/
	cgi.R_AddLightToScene(end, 225.0f, 0.7f, 0.7f, 0.8f);
}
/*
...
{ //this also sets the lerp frames, so I suggest you keep calling it regardless of if you want anim sounds.
	...
	//cent->pe.legs.oldFrameTime = cent->pe.legs.frameTime;
	...
	//cent->pe.torso.oldFrameTime = cent->pe.torso.frameTime;
	...
	if (cent->ghoul2)
	{
		...
		{ //rww - The guesswork based on the lerp frame figures is usually BS, so I've resorted to a call to get the frame of the bone directly.
			...
			for(i = 0; i < ARMSLOT_MAX; i++)
			{
				if(cent->armorGhoul2[i] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[i]))
				{
					/*if (resumeFrame)
					{ //we already checked, and this is the same anim, same flip state, but different speed, so we want to resume with the new speed off of the same frame.
						cgi.G2API_GetBoneFrame(cent->armorGhoul2[i], "lower_lumbar", cg.time, &GBAcFrame, NULL, 0);
						beginFrame = GBAcFrame;
					}

					//even if resuming, also be sure to check if we are running the same frame on the legs. If so, we want to use their frame no matter what.
					cgi.G2API_GetBoneFrame(cent->armorGhoul2[i], "model_root", cg.time, &GBAcFrame, NULL, 0);
					...
					cgi.G2API_SetBoneAnim(cent->armorGhoul2[i], 0, "lower_lumbar", firstFrame, lastFrame, flags, animSpeed,cg.time, beginFrame, blendTime);
				}
			}
		...
		else
		{
			...
			for(i = 0; i < ARMSLOT_MAX; i++)
			{
				if(cent->armorGhoul2[i] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[i]))
				{
					...
					cgi.G2API_SetBoneAnim(cent->armorGhoul2[i], 0, "model_root", firstFrame, lastFrame, flags, animSpeed, cg.time, beginFrame, blendTime);
				}
			}
		...
		{ //make sure we're humanoid before we access the motion bone
			...
			for(i = 0; i < ARMSLOT_MAX; i++)
			{
				if(cent->armorGhoul2[i] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[i]))
				{
					cgi.G2API_SetBoneAnim(cent->armorGhoul2[i], 0, "Motion", firstFrame, lastFrame, flags, animSpeed, cg.time, beginFrame, blendTime);
				}
			}
		}
...
static void CG_RunLerpFrame( centity_t *cent, clientInfo_t *ci, lerpFrame_t *lf, qboolean flipState, int newAnimation, float speedScale, qboolean torsoOnly) 
{
	...
	if (cent->currentState.forceFrame)
	{
		if (lf->lastForcedFrame != cent->currentState.forceFrame)
		{
			...
			for(i = 0; i < ARMSLOT_MAX; i++)
			{
				if(cent->armorGhoul2[i] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[i]))
				{
					cgi.G2API_SetBoneAnim(cent->armorGhoul2[i], 0, "lower_lumbar", cent->currentState.forceFrame, cent->currentState.forceFrame+1, flags, animSpeed, cg.time, -1, 150);
					cgi.G2API_SetBoneAnim(cent->armorGhoul2[i], 0, "model_root", cent->currentState.forceFrame, cent->currentState.forceFrame+1, flags, animSpeed, cg.time, -1, 150);
					cgi.G2API_SetBoneAnim(cent->armorGhoul2[i], 0, "Motion", cent->currentState.forceFrame, cent->currentState.forceFrame+1, flags, animSpeed, cg.time, -1, 150);
				}
			}
		}
	...
	else if ( JKG_DamageTypeFreezes ((const damageType_t)cent->currentState.damageTypeFlags) )
	{
	    ...
	    if ( !torsoOnly )
	    {
	        ...
	        cgi.G2API_SetBoneAnim (cent->ghoul2, 0, "model_root", legsAnimFrame, legsAnimFrame, BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, 1.0f, cg.time, -1, 150);
			for(i = 0; i < ARMSLOT_MAX; i++)
			{
				if(cent->armorGhoul2[i] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[i]))
				{
					cgi.G2API_SetBoneAnim (cent->armorGhoul2[i], 0, "model_root", legsAnimFrame, legsAnimFrame, BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, 1.0f, cg.time, -1, 150);
				}
			}
	    }
	    
	    cgi.G2API_SetBoneAnim (cent->ghoul2, 0, "lower_lumbar", torsoAnimFrame, torsoAnimFrame, BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, 1.0f, cg.time, -1, 150);
		for(i = 0; i < ARMSLOT_MAX; i++)
		{
			if(cent->armorGhoul2[i] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[i]))
			{
				cgi.G2API_SetBoneAnim (cent->armorGhoul2[i], 0, "lower_lumbar", torsoAnimFrame, torsoAnimFrame, BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, 1.0f, cg.time, -1, 150);
			}
		}
	}
	else
...
void	CG_Rag_Trace( trace_t *result, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, 
	...
	cgi.CM_BoxTrace ( result, start, end, mins, maxs, 0, mask);
...
#elif 1 //with my new method of doing things I want it to continue the anim
		{
			...
			{ //lock the anim on the current frame.
				...
				for(i = 0; i < ARMSLOT_MAX; i++)
				{
					if(cent->armorGhoul2[i] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[i]))
					{
						cgi.G2API_SetBoneAnim(cent->armorGhoul2[i], 0, "lower_lumbar", currentFrame, currentFrame+1, flags, animSpeed,cg.time, currentFrame, blendTime);
						cgi.G2API_SetBoneAnim(cent->armorGhoul2[i], 0, "model_root", currentFrame, currentFrame+1, flags, animSpeed, cg.time, currentFrame, blendTime);
						cgi.G2API_SetBoneAnim(cent->armorGhoul2[i], 0, "Motion", currentFrame, currentFrame+1, flags, animSpeed, cg.time, currentFrame, blendTime);
					}
				}
			}
		}
...
//	if (!animatingHead || ( animations[anim].firstFrame != startFrame ) )// only set the anim if we aren't going to do the same animation again
	...
	//		firstFrame, lastFrame, animFlags, animSpeed, cg.time, -1, blendTime);
		...
		for(i = 0; i < ARMSLOT_MAX; i++)
		{
			if(cent->armorGhoul2[i] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[i]))
			{
				cgi.G2API_SetBoneAnim(cent->armorGhoul2[i], 0, "face", firstFrame, lastFrame, animFlags, animSpeed, cg.time, -1, blendTime);
			}
		}
	}
}
...
static void CG_G2PlayerAngles( centity_t *cent, vec3_t legs[3], vec3_t legsAngles)
{
	...
	{ //don't do these things on non-humanoids
		...
		}
		{
			...
			for(i = 0; i < ARMSLOT_MAX; i++)
			{
				if(cent->armorGhoul2[i])
				{
					qhandle_t tempMotionBolt = cgi.G2API_AddBolt(cent->armorGhoul2[i], 0, "Motion");
					BG_G2PlayerAngles(cent->armorGhoul2[i], tempMotionBolt, &cent->currentState, cg.time,
						cent->lerpOrigin, cent->lerpAngles, legs, legsAngles, &cent->pe.torso.yawing, &cent->pe.torso.pitching,
						&cent->pe.legs.yawing, &cent->pe.torso.yawAngle, &cent->pe.torso.pitchAngle, &cent->pe.legs.yawAngle,
						cg.frametime, cent->turAngles, cent->modelScale, ci->legsAnim, ci->torsoAnim, &ci->corrTime,
						lookAngles, ci->lastHeadAngles, ci->lookTime, emplaced, &ci->superSmoothTime, cent->currentState.saberActionFlags);
...
static qboolean CG_PlayerShadow( centity_t *cent, float *shadowPlane ) {
	...
	*shadowPlane = 0;
	...
	{ //stencil
		...
		cgi.CM_BoxTrace( &trace, cent->lerpOrigin, end, mins, maxs, 0, MASK_PLAYERSOLID );
...
void CG_AddRefEntityToSceneWithShadows( centity_t *cent, refEntity_t legs )
{
	...
	{// Render each recorded shadow...
		...
		cgi.R_AddRefEntityToScene( &new_legs[i] );
	}
...
void CG_DoSaber( vec3_t origin, vec3_t dir, float length, float lengthMax, float radius, unsigned char crystal, int rfx, qboolean doLight )
{
	...
	if(!Q_stricmp(saberCrystalsLookup[crystal].glowEffect, "NULL_GLOW") ||
 	...
 	{
		glow = cgi.R_RegisterShader("gfx/lightsabers/rgb_glow");
	}
	else
	{
		glow = cgi.R_RegisterShader(va("gfx/lightsabers/rgb_glow_%s", saberCrystalsLookup[crystal].glowEffect));
	}

	if(!Q_stricmp(saberCrystalsLookup[crystal].bladeEffect, "NULL_BLADE") ||
		!saberCrystalsLookup[crystal].bladeEffect[0])
	{
		blade = cgi.R_RegisterShader("gfx/lightsabers/rgb_blade_dull");
	}
	else
	{
		blade = cgi.R_RegisterShader(va("gfx/lightsabers/rgb_blade_%s", saberCrystalsLookup[crystal].bladeEffect));
	}
...
//	byte			colors[4];
	...
	// get the fragments
	numFragments = cgi.CM_MarkFragments( 4, (const float (*)[3])originalPoints,
...
qboolean CG_G2TraceCollide(trace_t *tr, vec3_t const mins, vec3_t const maxs, const vec3_t lastValidStart, const vec3_t lastValidEnd)
{
	...
	if (g2Hit && g2Hit->ghoul2)
	{
		...
		if (cg_optvehtrace.integer &&
			...
			g2Hit->m_pVehicle)
		{
			cgi.G2API_CollisionDetectCache ( G2Trace, g2Hit->ghoul2, angles, g2Hit->lerpOrigin, cg.time, g2Hit->currentState.number, lastValidStart, lastValidEnd, g2Hit->modelScale, 0, cg_g2TraceLod.integer, fRadius );
		}
		else
		{
			cgi.G2API_CollisionDetect ( G2Trace, g2Hit->ghoul2, angles, g2Hit->lerpOrigin, cg.time, g2Hit->currentState.number, lastValidStart, lastValidEnd, g2Hit->modelScale, 0, cg_g2TraceLod.integer, fRadius );
		}
	...
	while (!doneWithTraces)
	{
		...
		{ //hit a client..
			...
			{ //it succeeded with the ghoul2 trace
				cgi.FX_PlayEffectID( cgs.effects.mSaberBloodSparks, trace.endpos, trace.plane.normal, -1, -1 );
		...
		{ //this is the guy the server says we last hit, so continue.
			...
			if (doEffect)
			{
				...
				{ //do standard player/live ent hit sparks
					cgi.FX_PlayEffectID( hitPersonFxID, trace.endpos, trace.plane.normal, -1, -1 );
					//cgi.S_StartSound(trace.endpos, trace.entityNum, CHAN_AUTO, cgi.S_RegisterSound(va("sound/weapons/saber/saberhit%i.wav", Q_irand(1, 3))));
				}
				else
				{ //do the cut effect
					cgi.FX_PlayEffectID( hitOtherFxID, trace.endpos, trace.plane.normal, -1, -1 );
				}
				doEffect = qfalse;
			}
		}
		...
		for ( i = 0; i < 1; i++ )//was 2 because it would go through architecture and leave saber trails on either side of the brush - but still looks bad if we hit a corner, blade is still 8 longer than hit
		{
			...
			if ( trace.fraction < 1.0f )
			{
				...
				else
				{
					...
					{
						cgi.FX_PlayEffectID( cgs.effects.mSparks, trace.endpos, trDir, -1, -1 );
					}
				}
					...
					{ //don't draw it if the last time is way out of date
						...
						else
						{
							...
							*/
							...
							{
								fx.mShader = cgi.R_RegisterShader(va("gfx/lightsabers/rgb_trail_%s", saberCrystalsLookup[cent->currentState.saberCrystal[saberNum]].trailEffect));
 							}
							else
							{
								fx.mShader = cgs.media.saberBlurShader;
						...
						if (cg_saberTrail.integer == 2 && cg_shadows.integer != 2 && cgs.glconfig.stencilBits >= 4)
						{
							cgi.R_SetRefractProp(1.0f, 0.0f, qtrue, qtrue); //don't need to do this every frame.. but..
	...
	if ( pVehNPC->m_pVehicleInfo->type != VH_ANIMAL )
	{
		...
		{//until we attach it, we need to debounce this
			...
			{//FIXME: only do this when accelerator is being pressed! (must have a driver?)
				...
				if (0)
				{//hurt
					cgi.FX_PlayEffectID( cgs.effects.mBlackSmoke, org, fwd, -1, -1 );
				}
				else if ( pVehNPC->m_pVehicleInfo->iTrailFX )
				{//okay, do normal trail
					cgi.FX_PlayEffectID( pVehNPC->m_pVehicleInfo->iTrailFX, org, fwd, -1, -1 );
				}
				//=====================================================================
				//EXHAUST FX
				//=====================================================================
				...
				if ( doExhaust && cent->ghoul2 )
				{
					...
					for ( i = 0; i < MAX_VEHICLE_EXHAUSTS; i++ )
					{
						...
						{ //fixme: bolt these too
							...
							cgi.FX_PlayEffectID( fx, boltOrg, boltDir, -1, -1 );
						}
					}
				}
				//=====================================================================
				...
				if ( pVehNPC->m_pVehicleInfo->iTrailFX != 0 && cent->ghoul2 )
				{
					...
					for ( i = 1; i < 5; i++ )
					{
						...
						cgi.FX_PlayEffectID( pVehNPC->m_pVehicleInfo->iTrailFX, boltOrg, boltDir, -1, -1 );
					}
				}
			}
			//FIXME armor needs to be sent over network
			{
				...
				{//just plain dead, use flames 
					...
					//	BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, boltOrg);
					//}
					cgi.FX_PlayEffectID( cgs.effects.mShipDestBurning, boltOrg, up, -1, -1 );
				}
			}
			...
			{//FIXME: should use something attached.. but want it to build up over time, so...
				...
				{//flaming!
					...
					VectorScale( fwd, -1, fwd );
					cgi.FX_PlayEffectID( cgi.FX_RegisterEffect("ships/fire"), org, fwd, -1, -1 );
					nextFXDelay = 50;
				}
			}
			*/
...
void CG_DrawPlayerBBox ( const centity_t *cent )
{
    ...
    cgi.R_AddPolysToScene (cgs.media.bboxShader, 4, &verts[0], 6);
}
...
void CG_Player( centity_t *cent ) {
	...
	//eezstreet add: for armor
	refEntity_t		armorG2[ARMSLOT_MAX];
	...
	// TODO: Fading, behind-quick-fade for NPC. Enable when completed.
	/*if (( int ) VectorLength( difference ) > 1024.0f )
	{
		if ( cent->fadeState == 3 )
		{
			return;
		...
		cent->fadeState = 2;
		cent->fadeAlpha = cg.time + 1000;
	}
	else if ( cent->fadeState == 3 )
	{
		cent->fadeState = 1;
		cent->fadeAlpha = cg.time + 1000;
	}*/
	...
	{ //eezstreet add
		...
		{ //do crazy smoothing
			...
			{ //do it
				...
				for(ijk = 0; ijk < ARMSLOT_MAX; ijk++)
				{
					if(cent->armorGhoul2[ijk] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[ijk]))
					{
						cgi.G2API_AbsurdSmoothing(cent->armorGhoul2[ijk], qtrue);
					}
				}
			}
			else
			{ //turn it off
				...
				for(ijk = 0; ijk < ARMSLOT_MAX; ijk++)
				{
					if(cent->armorGhoul2[ijk] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[ijk]))
					{
						cgi.G2API_AbsurdSmoothing(cent->armorGhoul2[ijk], qfalse);
					}
				}
			}
		}
		...
		if (cent->currentState.eFlags & EF_JETPACK_ACTIVE)
		{
			...
			while (n < 2)
			{
				...
				{ //create effects
					...
					//Play the effect
					cgi.FX_PlayEffectID(cgs.effects.mJetpack, flamePos, flameDir, -1, -1);
					cgi.FX_PlayEffectID(cgs.effects.mJetpack, flamePos, flameDir, -1, -1);

					//Keep the jet fire sound looping
					cgi.S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, 
						cgi.S_RegisterSound( "sound/jkg/jetpack/jetlp" /*"sound/effects/fire_lp"*/ ) );
				}
				else
				{ //just idling
			...
			cgi.S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, 
						cgi.S_RegisterSound( "sound/jkg/jetpack/jethover" /*"sound/effects/fire_lp"*/ ) );
			//cgi.S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, 
...
//[TrueView]
	...
	if (cg.snap && cent->currentState.number == cg.snap->ps.clientNum)
	{
			...
			|| cent->currentState.weapon == WP_MELEE) && !cg.predictedPlayerState.zoomMode && !cg.cinematicState)	
		{
			...
			eyesBolt = cgi.G2API_AddBolt(cent->ghoul2, 0, "*head_eyes");
			if( !cgi.G2API_GetBoltMatrix(cent->ghoul2, 0, eyesBolt, &eyeMatrix, cent->turAngles, cent->lerpOrigin, 
				cg.time, cgs.gameModels, cent->modelScale) )
			{//Something prevented you from getting the "*head_eyes" information.  The model probably doesn't have a 
				...
				eyesBolt = cgi.G2API_AddBolt(cent->ghoul2, 0, "*head_front");
				if( !cgi.G2API_GetBoltMatrix(cent->ghoul2, 0, eyesBolt, &eyeMatrix, cent->turAngles, cent->lerpOrigin, 
					cg.time, cgs.gameModels, cent->modelScale) )
				{
					eyesBolt = cgi.G2API_AddBolt(cent->ghoul2, 0, "reye");
					boneBased = qtrue;
					if( !cgi.G2API_GetBoltMatrix(cent->ghoul2, 0, eyesBolt, &eyeMatrix, cent->turAngles, cent->lerpOrigin, 
					cg.time, cgs.gameModels, cent->modelScale) )
					{
	...
	*/
		...
		cent->frame_minus2_refreshed)
	{
		...
		if (cent->frame_minus1_refreshed)
		{
			...
			for(ijk = 0; ijk < ARMSLOT_MAX; ijk++)
			{
				if(cent->armorGhoul2[ijk] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[ijk]))
				{
					reframe_minus1.ghoul2 = cent->armorGhoul2[ijk];
					cgi.R_AddRefEntityToScene(&reframe_minus1);
				}
			}
		}
		...
		if (cent->frame_minus2_refreshed)
		{
			...
			for(ijk = 0; ijk < ARMSLOT_MAX; ijk++)
			{
				if(cent->armorGhoul2[ijk] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[ijk]))
				{
					reframe_minus2.ghoul2 = cent->armorGhoul2[ijk];
					cgi.R_AddRefEntityToScene(&reframe_minus2);
				}
			}
		}
	...
	{//doing the electrocuting
		...
		{//arc
			...
			{
				cgi.FX_PlayEntityEffectID(cgs.effects.forceLightningWide, efOrgR, axis, -1, -1, -1, -1); // thats the solution - we need to play the fx from two origins
			}

			cgi.FX_PlayEntityEffectID(cgs.effects.forceLightningWide, efOrgL, axis, -1, -1, -1, -1);
		}
		else
		{//line
			//cgi.FX_PlayEffectID( cgs.effects.forceLightning, efOrg, fxDir );
			//cgi.FX_PlayEntityEffectID(cgs.effects.forceLightning, efOrg, axis, cent->boltInfo, cent->currentState.number, -1, -1);
			cgi.FX_PlayEntityEffectID(cgs.effects.forceLightning, efOrgL, axis, -1, -1, -1, -1);
		}
	...
	//	{
	//		cent->fadeAlpha += 5;
	//		CG_Printf( "Fading In %i..\n", cent->fadeAlpha );
	//		if ( cent->fadeAlpha >= 255 ) cent->fadeState = 0;
	//	}
	//	/* Fading Out */
	//	else
	//	{
	//		cent->fadeAlpha -= 5;
	//		CG_Printf( "Fading Out %i..\n", cent->fadeAlpha );
	//		if ( cent->fadeAlpha <= 0 ) cent->fadeState = 3;
	//	}

	//	legs.renderfx		|= RF_FORCE_ENT_ALPHA; // (RF_MINLIGHT | RF_ALPHA_DEPTH | RF_FORCE_ENT_ALPHA );
	//	legs.shaderRGBA[3]  = cent->fadeAlpha;
	//}
...
stillDoSaber:
	...
	if (drawPlayerSaber)
	{
		...
		else if (!cent->currentState.saberInFlight)
		{
			...
			{
				cgi.G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_SABER, 0), 0, cent->ghoul2, 1);
		...
		else
		{
			if (cg.renderingThirdPerson || cent->currentState.number != cg.predictedPlayerState.clientNum)
			{
				...
				*/
				...
				{
					cgi.R_SetRefractProp(1.0f, 0.0f, qfalse, qfalse); //don't need to do this every frame.. but..
	...
	// Render flashlight if applicable
	CG_DoFieldLight( cent );
...
endOfCall:
	...
	//It's time for Fat Man to make his debut -- eezstreet
	{
		int ijk;
		for(ijk = 0; ijk < ARMSLOT_MAX; ijk++)
		{
			if( cent && cent->ghoul2 && cent->armorGhoul2[ijk] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[ijk]) && cent->currentState.eType != ET_NPC)
			{
				armorG2[ijk].ghoul2 = cent->armorGhoul2[ijk];
				armorG2[ijk].shadowPlane = shadowPlane;		// Note: these two lines are left out of the below check because they need to be shown in first person in order
															// for stencil shadows to show up properly in first person --eez
				//armorG2[ijk].renderfx &= ~RF_FORCE_ENT_ALPHA;
				//armorG2[ijk].renderfx |= RF_RGB_TINT;
				armorG2[ijk].shaderRGBA[3] = 0;
				cgi.R_AddRefEntityToScene( &armorG2[ijk] );
			}
			if (cent && cent->ghoul2 && cent->armorGhoul2[ijk] && cgi.G2_HaveWeGhoul2Models(cent->armorGhoul2[ijk]) && cent->currentState.eType != ET_NPC && !(!cg.renderingThirdPerson && cent->currentState.number == cg.predictedPlayerState.clientNum))
			{

				armorG2[ijk].shaderRGBA[3] = 255;
				if (cent->uncloaking > cg.time)
				{//in the middle of cloaking
					if ((cg.snap->ps.fd.forcePowersActive & (1 << FP_SEE))
					   && cg.snap->ps.clientNum != cent->currentState.number)
					...
					else
					{
						...
						if ( perc >= 0.0f && perc <= 1.0f )
						{
								armorG2[ijk].renderfx &= ~RF_FORCE_ENT_ALPHA;
								armorG2[ijk].renderfx |= RF_RGB_TINT;
								armorG2[ijk].shaderRGBA[0] = armorG2[ijk].shaderRGBA[1] = armorG2[ijk].shaderRGBA[2] = 255.0f * perc;
								armorG2[ijk].shaderRGBA[3] = 0;
								armorG2[ijk].customShader = cgs.media.cloakedShader;
								cgi.R_AddRefEntityToScene( &armorG2[ijk] );

								armorG2[ijk].shaderRGBA[0] = armorG2[ijk].shaderRGBA[1] = armorG2[ijk].shaderRGBA[2] = 255;
								armorG2[ijk].shaderRGBA[3] = 255 * (1.0f - perc); // let model alpha in
								armorG2[ijk].customShader = 0; // use regular skin
								armorG2[ijk].renderfx &= ~RF_RGB_TINT;
								armorG2[ijk].renderfx |= RF_FORCE_ENT_ALPHA;
								cgi.R_AddRefEntityToScene( &armorG2[ijk] );
						}
				...
				{//fully cloaked
					...
					else
					{
						...
						{ //stencil buffer's in use, sorry
							armorG2[ijk].renderfx = 0;//&= ~(RF_RGB_TINT|RF_ALPHA_FADE);
							armorG2[ijk].shaderRGBA[0] = armorG2[ijk].shaderRGBA[1] = armorG2[ijk].shaderRGBA[2] = armorG2[ijk].shaderRGBA[3] = 255;
							armorG2[ijk].customShader = cgs.media.cloakedShader;
							cgi.R_AddRefEntityToScene( &armorG2[ijk] );
							armorG2[ijk].customShader = 0;
						}
			...
			if (!(cent->currentState.powerups & (1 << PW_CLOAKED)))
			{ //don't add the normal model if cloaked
				CG_CheckThirdPersonAlpha( cent, &armorG2[ijk] );
				cgi.R_AddRefEntityToScene( &armorG2[ijk] );
			}
			...
			if ((cent->currentState.forcePowersActive & (1 << FP_RAGE)) &&
				(cg.renderingThirdPerson || cent->currentState.number != cg.snap->ps.clientNum))
			{
				//armorG2[ijk].customShader = cgs.media.rageShader;
				armorG2[ijk].renderfx &= ~RF_FORCE_ENT_ALPHA;
				armorG2[ijk].renderfx &= ~RF_MINLIGHT;

				armorG2[ijk].renderfx |= RF_RGB_TINT;
				armorG2[ijk].shaderRGBA[0] = 255;
				armorG2[ijk].shaderRGBA[1] = armorG2[ijk].shaderRGBA[2] = 0;
				armorG2[ijk].shaderRGBA[3] = 255;
	            
				if ( rand() & 1 )
				{
					armorG2[ijk].customShader = cgs.media.electricBodyShader;   
				}
				else
				{
					armorG2[ijk].customShader = cgs.media.electricBody2Shader;
				}
			...
			if (!cg.snap->ps.duelInProgress && cent->currentState.bolt1 && !(cent->currentState.eFlags & EF_DEAD) && cent->currentState.number != cg.snap->ps.clientNum && (!cg.snap->ps.duelInProgress || cg.snap->ps.duelIndex != cent->currentState.number))
			{
				...
				armorG2[ijk].renderfx &= ~RF_RGB_TINT;
				armorG2[ijk].renderfx &= ~RF_FORCE_ENT_ALPHA;
				armorG2[ijk].customShader = cgs.media.forceSightBubble;
			...
			{ //aborb is represented by green..
				...
				memcpy(&prot, &armorG2[ijk], sizeof(prot));
			...
			if ((cent->currentState.number == cg.predictedPlayerState.clientNum && (cg.predictedPlayerState.fd.forcePowersActive & (1<<FP_ABSORB))) ||
				(cent->teamPowerEffectTime > cg.time && cent->teamPowerType == 3))
			{ //aborb is represented by blue..
				armorG2[ijk].shaderRGBA[0] = 0;
				armorG2[ijk].shaderRGBA[1] = 0;
				armorG2[ijk].shaderRGBA[2] = 255;
				armorG2[ijk].shaderRGBA[3] = 254;
			...
			if ((cg.snap->ps.fd.forcePowersActive & (1 << FP_SEE)) && cg.snap->ps.clientNum != cent->currentState.number && cg_auraShell.integer)
			{
				...
				{   // A team game
					switch(ci->team)
					{
						case TEAM_RED:
							...
							break;
						case TEAM_BLUE:
							armorG2[ijk].shaderRGBA[0] = 75;
							armorG2[ijk].shaderRGBA[1] = 75;
							armorG2[ijk].shaderRGBA[2] = 255;
							break;
				...
				{   // See through walls.
					armorG2[ijk].renderfx |= RF_MINLIGHT | RF_NODEPTH;
				...
				armorG2[ijk].renderfx &= ~RF_RGB_TINT;
				armorG2[ijk].renderfx &= ~RF_FORCE_ENT_ALPHA;
				armorG2[ijk].customShader = cgs.media.sightShell;
			...
			if ( cent->currentState.emplacedOwner > cg.time )
			{
				...
				if ( dif > 0 && random() > 0.4f )
				{
					...
					armorG2[ijk].renderfx |= RF_RGB_TINT;
					armorG2[ijk].shaderRGBA[0] = armorG2[ijk].shaderRGBA[1] = armorG2[ijk].shaderRGBA[2] = brightness;
					armorG2[ijk].shaderRGBA[3] = 255;
				...
				VectorSet(tempAngles, 0, cent->lerpAngles[YAW], 0);
				CG_ForceElectrocution( cent, armorG2[ijk].origin, tempAngles, cgs.media.boltShader, qfalse );
			}

			if (cent->currentState.powerups & (1 << PW_SHIELDHIT))
			{
				armorG2[ijk].shaderRGBA[0] = armorG2[ijk].shaderRGBA[1] = armorG2[ijk].shaderRGBA[2] = Q_irand(1, 255);
