...
int Cin_ProcessCamera() {
	...
	CamData_Union_t *CamData = cin.CamData;
	centity_t *ent;
	vec3_t temp;
	int i;
	int pt = 0;
...
static int Cin_ProcessCM_Scale(ppColormod_t *cm) {
	if (!cin.CM.scale.state) {
		cm->red_scale = 1;
		cm->green_scale = 1;
		cm->blue_scale = 1;
		return 0;
	...
	if (cin.CM.scale.state == 2) {
		// Static
		cm->red_scale = cin.CM.scale.from[0];
		cm->green_scale = cin.CM.scale.from[1];
		cm->blue_scale = cin.CM.scale.from[2];
		return 1;
	}
	if (cin.CM.scale.state == 1) {
		// Fading
		float fade;
		float delta;
		if (cg.time > cin.CM.scale.startTime + cin.CM.scale.duration) {
			if (cin.CM.scale.to[0] == 1 && cin.CM.scale.to[1] == 1 && cin.CM.scale.to[2] == 1) {
				...
				VectorCopy(cin.CM.scale.to, cin.CM.scale.from);
				cm->red_scale = cin.CM.scale.from[0];
				cm->green_scale = cin.CM.scale.from[1];
				cm->blue_scale = cin.CM.scale.from[2];
				return 1;
			...
			if (cg.time < cin.CM.scale.startTime) {
				fade = 0;
			} else {
				fade = ((float)(cg.time - cin.CM.scale.startTime) / cin.CM.scale.duration);
			}
			delta = (cin.CM.scale.to[0] - cin.CM.scale.from[0]) * fade;
			cm->red_scale = (cin.CM.scale.from[0] + delta);
			delta = (cin.CM.scale.to[1] - cin.CM.scale.from[1]) * fade;
			cm->green_scale = (cin.CM.scale.from[1] + delta);
			delta = (cin.CM.scale.to[2] - cin.CM.scale.from[2]) * fade;
			cm->blue_scale = (cin.CM.scale.from[2] + delta);
		}
		return 1;
	}
	trap_Error(va("ERROR: Cin_ProcessCM_Scale: Invalid state %i", cin.CM.scale.state));
	return 0;
}

static int Cin_ProcessCM_Bias(ppColormod_t *cm) {
	if (!cin.CM.bias.state) {
		cm->red_bias = 0;
		cm->green_bias = 0;
		cm->blue_bias = 0;
		return 0;
	}
	if (cin.CM.bias.state == 2) {
		// Static
		cm->red_bias = cin.CM.bias.from[0];
		cm->green_bias = cin.CM.bias.from[1];
		cm->blue_bias = cin.CM.bias.from[2];
		return 1;
	}
	if (cin.CM.bias.state == 1) {
		// Fading
		float fade;
		float delta;
		if (cg.time > cin.CM.bias.startTime + cin.CM.bias.duration) {
			if (cin.CM.bias.to[0] == 1 && cin.CM.bias.to[1] == 1 && cin.CM.bias.to[2] == 1) {
				cin.CM.bias.state = 0;
				cm->red_bias = 0;
				cm->green_bias = 0;
				cm->blue_bias = 0;
				...
				cin.CM.bias.state = 2;
				VectorCopy(cin.CM.bias.to, cin.CM.bias.from);
				cm->red_bias = cin.CM.bias.from[0];
				cm->green_bias = cin.CM.bias.from[1];
				cm->blue_bias = cin.CM.bias.from[2];
				return 1;
			...
			if (cg.time < cin.CM.bias.startTime) {
				fade = 0;
			} else {
				fade = ((float)(cg.time - cin.CM.bias.startTime) / cin.CM.bias.duration);
			}
			delta = (cin.CM.bias.to[0] - cin.CM.bias.from[0]) * fade;
			cm->red_bias = (cin.CM.bias.from[0] + delta);
			delta = (cin.CM.bias.to[1] - cin.CM.bias.from[1]) * fade;
			cm->green_bias = (cin.CM.bias.from[1] + delta);
			delta = (cin.CM.bias.to[2] - cin.CM.bias.from[2]) * fade;
			cm->blue_bias = (cin.CM.bias.from[2] + delta);
		}
		return 1;
	}
	trap_Error(va("ERROR: Cin_ProcessCM_Bias: Invalid state %i", cin.CM.bias.state));
	return 0;
}

static int Cin_ProcessCM_BC(ppColormod_t *cm) {
	if (!cin.CM.bc.state) {
		cm->brightness = 0;
		cm->contrast = 1;
		return 0;
	}
	if (cin.CM.bc.state == 2) {
		// Static
		cm->brightness = cin.CM.bc.from[0];
		cm->contrast = cin.CM.bc.from[1];
		return 1;
	}
	if (cin.CM.bc.state == 1) {
		// Fading
		float fade;
		float delta;
		if (cg.time > cin.CM.bc.startTime + cin.CM.bc.duration) {
			if (cin.CM.bc.to[0] == 0 && cin.CM.bc.to[1] == 1) {
				cin.CM.bc.state = 0;
				cm->brightness = 0;
				cm->contrast = 1;
				return 0;
				...
				cin.CM.bc.state = 2;
				Vector2Copy(cin.CM.bc.to, cin.CM.bc.from);
				cm->brightness = cin.CM.bc.from[0];
				cm->contrast = cin.CM.bc.from[1];
				return 1;
			}
		} else {
			if (cg.time < cin.CM.bc.startTime) {
				fade = 0;
			} else {
				fade = ((float)(cg.time - cin.CM.bc.startTime) / cin.CM.bc.duration);
			}
			delta = (cin.CM.bc.to[0] - cin.CM.bc.from[0]) * fade;
			cm->brightness = (cin.CM.bc.from[0] + delta);
			delta = (cin.CM.bc.to[1] - cin.CM.bc.from[1]) * fade;
			cm->contrast = (cin.CM.bc.from[1] + delta);
		}
		return 1;
	}
	trap_Error(va("ERROR: Cin_ProcessCM_BC: Invalid state %i", cin.CM.bc.state));
	return 0;
}

static int Cin_ProcessCM_FX(ppColormod_t *cm) {
	if (!cin.CM.fx.state) {
		cm->fx = 0;
		cm->fxintensity = 0;
		cm->fxbrightness = 1;
	...
	if (cin.CM.fx.state == 2) {
		// Static
		cm->fx = cin.CM.fx.fxID;
		cm->fxintensity = cin.CM.fx.from[0];
		cm->fxbrightness = cin.CM.fx.from[1];
		return 1;
	}
	if (cin.CM.fx.state == 1) {
		// Fading
		float fade;
		float delta;
		if (cg.time > cin.CM.fx.startTime + cin.CM.fx.duration) {
			if (cin.CM.fx.to[0] == 0) {
				cin.CM.fx.state = 0;
				cm->fx = 0;
				cm->fxintensity = 0;
				...
				cin.CM.fx.state = 2;
				Vector2Copy(cin.CM.fx.to, cin.CM.fx.from);
				cm->fx = cin.CM.fx.fxID;
				cm->fxintensity = cin.CM.fx.from[0];
				cm->fxbrightness = cin.CM.fx.from[1];
				return 1;
			...
			cm->fx = cin.CM.fx.fxID;
			if (cg.time < cin.CM.fx.startTime) {
				fade = 0;
			} else {
				fade = ((float)(cg.time - cin.CM.fx.startTime) / cin.CM.fx.duration);
			}
			delta = (cin.CM.fx.to[0] - cin.CM.fx.from[0]) * fade;
			cm->fxintensity = (cin.CM.fx.from[0] + delta);
			delta = (cin.CM.fx.to[1] - cin.CM.fx.from[1]) * fade;
			cm->fxbrightness = (cin.CM.fx.from[1] + delta);
		}
		return 1;
	}
	trap_Error(va("ERROR: Cin_ProcessCM_FX: Invalid state %i", cin.CM.fx.state));
	return 0;
}

static int Cin_ProcessCM_Inv(ppColormod_t *cm) {
	if (!cin.CM.inv.state) {
		cm->inversion = 0;
		return 0;
	}
	if (cin.CM.inv.state == 2) {
		// Static
		cm->inversion = cin.CM.inv.from;
		return 1;
	}
	if (cin.CM.inv.state == 1) {
		// Fading
		float fade;
		float delta;
		if (cg.time > cin.CM.inv.startTime + cin.CM.inv.duration) {
			if (cin.CM.inv.to == 0) {
				cin.CM.inv.state = 0;
				cm->inversion = 0;
				return 0;
				...
				cin.CM.inv.state = 2;
				cin.CM.inv.from = cin.CM.inv.to;
				cm->inversion = cin.CM.inv.from;
				return 1;
			}
		} else {
			if (cg.time < cin.CM.inv.startTime) {
				fade = 0;
			} else {
				fade = ((float)(cg.time - cin.CM.inv.startTime) / cin.CM.inv.duration);
			}
			delta = (cin.CM.inv.to - cin.CM.inv.from) * fade;
			cm->inversion = (cin.CM.inv.from + delta);
		}
		return 1;
	}
	trap_Error(va("ERROR: Cin_ProcessCM_Inv: Invalid state %i", cin.CM.inv.state));
	return 0;
}

void Cin_ProcessCM(ppColormod_t *cm) {
	int inUse = 0;
	memset(cm, 0, sizeof(ppColormod_t));
	inUse += Cin_ProcessCM_Scale(cm);
	inUse += Cin_ProcessCM_Bias(cm);
	inUse += Cin_ProcessCM_BC(cm);
	inUse += Cin_ProcessCM_FX(cm);
	inUse += Cin_ProcessCM_Inv(cm);
	cm->active = (inUse != 0);
}
...
void Cin_ProcessCinematicBinary_f() {
	...
	BitStream_Init(&stream, data, len);
	...
	for(;;) {
		...
		case CIN_ACT_LINEARCAM:
			...
			CamData->CamLinear.pointData = malloc(sizeof(CamData_Linear_Point_t) * CamData->CamLinear.pointcount);
		...
		case CIN_ACT_SPLINECAM:
			...
			CamData->CamSpline.Algorithms[0] = BitStream_ReadBits(&stream, 2);
			CamData->CamSpline.Algorithms[1] = BitStream_ReadBits(&stream, 2);
			CamData->CamSpline.Algorithms[2] = BitStream_ReadBits(&stream, 2);
			CamData->CamSpline.Algorithms[3] = BitStream_ReadBits(&stream, 2);
			CamData->CamSpline.Algorithms[4] = BitStream_ReadBits(&stream, 2);
			CamData->CamSpline.Algorithms[5] = BitStream_ReadBits(&stream, 2);

			// Alloc a buffer for the points
			CamData->CamSpline.pointData = malloc(sizeof(CamData_Spline_Point_t) * CamData->CamSpline.pointcount);
...
void Cin_ProcessCinematic_f() {
	...
	while (1) {
		...
		if (!Q_stricmp(token, "spc")) {
			...
			CamData->CamSpline.Algorithms[0] = (algos & 3072) >> 10;
			CamData->CamSpline.Algorithms[1] = (algos & 768) >> 8;
			CamData->CamSpline.Algorithms[2] = (algos & 192) >> 6;
			CamData->CamSpline.Algorithms[3] = (algos & 48) >> 4;
			CamData->CamSpline.Algorithms[4] = (algos & 12) >> 2;
			CamData->CamSpline.Algorithms[5] = (algos & 3);
...
static void Cin_DoLinearInterpolation() {
	...
	if (CamData->CamLinear.pointData[0].offset != 0) {
		...
		newarr = malloc(sizeof(CamData_Linear_Point_t) * (CamData->CamLinear.pointcount+1));
...
static void Cin_DoCubicSplineInterpolation() {
	...
	if (CamData->CamSpline.pointData[0].offset != 0) {
		...
		newarr = malloc(sizeof(CamData_Spline_Point_t) * (CamData->CamSpline.pointcount+1));
