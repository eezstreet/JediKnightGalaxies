	...
	{//go through all muzzles and 
		...
		{//this muzzle fired
			...
			if ( muzFX )
			{
				...
				//cgi.FX_PlayEffectID(muzFX, pVeh->m_vMuzzlePos[curMuz], pVeh->m_vMuzzleDir[curMuz], -1, -1);
...
//		cgi.S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.noAmmoSound );
	...
	case EV_CHANGE_WEAPON:
		DEBUGNAME("EV_CHANGE_WEAPON");
		{
			...
			{
				cgi.S_StartSound( NULL, es->number, CHAN_AUTO, cgi.S_RegisterSound(weaponData->visuals.selectSound) );
			}
	...
	case EV_RELOAD:
		DEBUGNAME("EV_RELOAD");
		{
			...
			{
				cgi.S_StartSound (NULL, es->time, CHAN_AUTO, weaponInfo->selectSound );
			}
			else
			{ //not sure what SP is doing for this but I don't want a select sound for saber (it has the saber-turn-on)
				cgi.S_StartSound (NULL, es->time, CHAN_AUTO, cgs.media.selectSound );
			}
	...
	case EV_FIRE_WEAPON:
		...
		{ //special case for turret firing
			...
			cgi.FX_PlayEffectID(cgs.effects.mEmplacedMuzzleFlash, gunpoint, gunangle, -1, -1);
		}
	...
	case EV_SABER_HIT:
		DEBUGNAME("EV_SABER_HIT");
		{
			...
			{ //Make lots of sparks, something special happened
				...
				cgi.FX_PlayEffectID( hitPersonFxID, es->origin, fxDir, -1, -1 );
				cgi.FX_PlayEffectID( hitPersonFxID, es->origin, fxDir, -1, -1 );
				cgi.FX_PlayEffectID( hitPersonFxID, es->origin, fxDir, -1, -1 );
				cgi.FX_PlayEffectID( hitPersonFxID, es->origin, fxDir, -1, -1 );
				cgi.FX_PlayEffectID( hitPersonFxID, es->origin, fxDir, -1, -1 );
			...
			{ //hit a person
				...
				{	// moderate or big hits.
					cgi.FX_PlayEffectID( hitPersonSmallFxID, es->origin, fxDir, -1, -1 );
				}
				else if ( es->eventParm == 2 )
				{	// this is for really big hits.
					cgi.FX_PlayEffectID( hitPersonMidFxID, es->origin, fxDir, -1, -1 );
				}
				else
			...
			{ //hit something else
				...
				cgi.FX_PlayEffectID( cgi.FX_RegisterEffect("saber/spark.efx"), es->origin, fxDir, -1, -1 );
				*/

				cgi.FX_PlayEffectID( hitOtherFxID, es->origin, fxDir, -1, -1 );
			}
		...
		DEBUGNAME("EV_SABER_BLOCK");
		{
			...
			{ //saber block
				...
				if (cullPass)
				{
					...
					cgi.FX_PlayEffectID( blockFXID, es->origin, fxDir, -1, -1 );
			...
			{ //projectile block
				...
				cgi.FX_PlayEffectID(cgs.effects.mBlasterDeflect, es->origin, fxDir, -1, -1);
			}
		}
		break;
	...
	case EV_BECOME_JEDIMASTER:
		DEBUGNAME("EV_SABER_UNHOLSTER");
		{
			...
			cgi.FX_PlayEffectID(cgs.effects.mJediSpawn, pos, ang, -1, -1);
...
// cleaned the below up. this was a huge mess before --eez
	...
	case EV_PLAYER_TELEPORT_IN:
		DEBUGNAME("EV_PLAYER_TELEPORT_IN");
		{
			...
			cgi.FX_PlayEffectID(cgs.effects.mSpawn, pos, ang, -1, -1);
		}
		break;
...
//		cgi.S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.missileStick );
	...
	case EV_MISSILE_HIT:
		...
		{//hack: this is an index to a custom effect to use
			cgi.FX_PlayEffectID(cgs.gameEffects[es->emplacedOwner], position, dir, -1, -1);
		}
	...
	case EV_PLAY_EFFECT:
		...
		if (eID != -1)
		{
			...
			cgi.FX_PlayEffectID(eID, es->origin, fxDir, -1, -1);
		}
		break;
	...
	case EV_PLAY_PORTAL_EFFECT_ID:
		DEBUGNAME("EV_PLAY_EFFECT_ID");
		{
			...
			if (efxIndex)
			{
				if (portalEffect)
				{
					cgi.FX_PlayPortalEffectID(efxIndex, position, fxDir, -1, -1 );
				}
				else
				{
					cgi.FX_PlayEffectID(efxIndex, position, fxDir, -1, -1 );
				}
			}
		}
		break;
		...
		if (es->groundEntityNum < MAX_CLIENTS && es->groundEntityNum >= 0)
		{
			...
			if (sfx)
			{
				...
				{ //add to the chat box
					...
					if (jkg_nokillmessages.integer!=1) {
					CG_Printf(vchatstr); //Disables rendering of kill messages as it is not needed in a MMO?
					}
	...
	case EV_WEAPON_CHARGE:
		...
		{
			cgi.S_StartSound(NULL, es->number, CHAN_WEAPON, weapon->chargeSound);
		}
	...
	case EV_WEAPON_CHARGE_ALT:
		...
		{
			cgi.S_StartSound(NULL, es->number, CHAN_WEAPON, weapon->altChargeSound);
		}*/
		break;
	...
	case EV_ITEM_BREAK:
		...
		cgi.S_StartSound(NULL, cg.snap->ps.clientNum, CHAN_AUTO, CGitemLookupTable[es->eventParm].itemType == ITEM_ARMOR ? cgs.media.armorBreakSound : cgs.media.weaponBreakSound);
		break;
