...
extern vmCvar_t		jkg_name;
extern vmCvar_t		jkg_dialcode;
extern vmCvar_t		jkg_serverid;
extern vmCvar_t		jkg_url;
...
static int JKG_NetworkHandShake( )
{
...
#ifdef _DEBUG
	G_Printf("DEBUG: Performing handshake\n");
#endif
	...
	curl_easy_setopt( jkg_connections.handshake, CURLOPT_URL, va("%s/authenticate.php", jkg_masterServerUrl.string) );
	...
	DH_getbigprime(dh, trap_Milliseconds());
	...
	if ( !success )
	{
	    ...
	    G_Printf ("Failed to generate keys. File %s, line %s.\n");
	...
	curl_easy_setopt( jkg_connections.handshake, CURLOPT_COPYPOSTFIELDS, va("r=%s&s=1", request));

	
	res = curl_easy_perform( jkg_connections.handshake );

	if (res != CURLE_OK) {
		G_Printf( "Libcurl Error while doing handshake: %s\n", curl_easy_strerror( res ) );
		// Free the DH context
		DH_free(dh);
	...
	if (httpcode == 400) {
		...
		trap_Error("Fatal error: Master server authentication failure!");
		return -3;
	}

	if ( stream.writeBuffer ) {
#ifndef FINAL_BUILD
		G_Printf( "^5%i bytes retrieved from the master server\n", stream.bufSize );
#endif
...
#ifndef FINAL_BUILD
		G_Printf( "^3JKG_NetworkTaskFunc: stream.writeBuffer is NULL but Libcurl returned CURLE_OK\n" );
#endif
		reply = NULL;
	}
...
#ifdef _DEBUG
	G_Printf("DEBUG: Handshake successful\n");
	G_Printf("DEBUG: Session ID: %s\n", jkg_auth.sessionID);
#endif
...
static int JKG_NetworkTaskFunc ( struct asyncTask_s *task )
{
	...
	curl_easy_setopt( socket->handle, CURLOPT_URL, va("%s/requestsv.php", jkg_masterServerUrl.string) );
...
{
	char *planetId = jkg_planetId.string;
	char buffer[1024];

	cJSONStream *stream = cJSON_Stream_New(8, 0, 1024, 512);

	va_list arg;
	va_start(arg, finalCallback);
	...
	switch ( method )
	{
		case LCMETHOD_TEST:
			...
			cJSON_Stream_WriteString( stream, "bounce", va("%i", level.time));
			break;
		case LCMETHOD_SVSTARTUP:
			// For G_InitGame, must pass level.time as 'time'
			cJSON_Stream_WriteString( stream, "do", "svstartup" );
			cJSON_Stream_WriteString( stream, "serverid", jkg_serverid.string );
			cJSON_Stream_WriteString( stream, "name", jkg_name.string );
			cJSON_Stream_WriteInteger( stream, "type", 0 );
			cJSON_Stream_WriteString( stream, "dialcode", jkg_dialcode.string );
			cJSON_Stream_WriteString( stream, "url", jkg_url.string );
			cJSON_Stream_WriteInteger( stream, "port", trap_Cvar_VariableIntegerValue("net_port") );

			trap_Cvar_VariableStringBuffer("mapname", buffer, sizeof(buffer));
			cJSON_Stream_WriteString( stream, "map", buffer );

			cJSON_Stream_WriteInteger( stream, "clients", 0 );
			cJSON_Stream_WriteInteger( stream, "maxclients", level.maxclients );

			cJSON_Stream_WriteInteger( stream, "location", 0 );
			cJSON_Stream_WriteInteger( stream, "instance", 0 );
			break;
		case LCMETHOD_SVSHUTDOWN:
			cJSON_Stream_WriteString( stream, "do", "svshutdown" );
			cJSON_Stream_WriteString( stream, "serverid", jkg_serverid.string );
			cJSON_Stream_WriteBoolean( stream, "keepsession", va_arg(arg, int) );
			break;
		case LCMETHOD_SVHEARTBEAT:
			cJSON_Stream_WriteString( stream, "do", "svheartbeat" );
			cJSON_Stream_WriteString( stream, "serverid", jkg_serverid.string );
			cJSON_Stream_WriteInteger( stream, "clients", level.numConnectedClients );
			break;
		case LCMETHOD_CLCONNECT:
			// For ClientConnect, must pass username as 'user', password hash as 'pass' and character slot as 'slot'
			cJSON_Stream_WriteString( stream, "do", "clconnect" );
			cJSON_Stream_WriteString( stream, "user", va_arg(arg, const char *) );
			cJSON_Stream_WriteString( stream, "pass", va_arg(arg, const char *) );
			cJSON_Stream_WriteInteger( stream, "slot", va_arg(arg, int) );
			break;
		case LCMETHOD_CLDISCONNECT:
			// For ClientDisconnect, must pass war.garbl as 'heh'
			cJSON_Stream_WriteString( stream, "do", "cldisconnect" );
			break;
		default:
	...
	return JKG_NewAsyncTask( JKG_NetworkTaskFunc, (void *)cJSON_Stream_Finalize(stream), TASKFLAG_MALLOCED_INITDATA, finalCallback );
}
...
#ifndef FINAL_BUILD
						G_Printf( "JKG_Libcurl_Pooler: stream.writeBuffer is NULL but Libcurl returned CURLE_OK\n" );
#endif
						reply = NULL;
					}
...
int JKG_Libcurl_Init ( void )
{
	...
	// Create a User-Agent string for Libcurl	
	trap_Cvar_VariableStringBuffer( "version", version, sizeof(version) );
	Com_sprintf( useragent, sizeof(useragent), "%s (%s; http://jkgalaxies.com/) %s", JKGTAG, version, curl_version() );
	...
	// Enable pipelining to further reduce overhead on the master server
	curl_multi_setopt(jkg_connections.multi, CURLMOPT_PIPELINING, 1);

#ifndef FINAL_BUILD
	Com_Printf( "Initialized Libcurl: %s\n", useragent );
	Com_Printf( "Master Server URL = %s\n", jkg_masterServerUrl.string );
	Com_Printf( "Planet ID = %i\n", jkg_planetId.integer );
#endif
...
void JKG_Libcurl_Shutdown ( void )
{
	...
	curl_easy_cleanup( curl_handle );	// TEMP
	curl_global_cleanup();

	Com_Printf( "Libcurl Shutdown\n" );
}
