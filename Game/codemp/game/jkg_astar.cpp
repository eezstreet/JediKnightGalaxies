...
extern "C"
{
	...
	int ASTAR_FindPath(int from, int to, int *pathlist)
	{
		...
		int temp_pathlist[MAX_WPARRAY_SIZE];
		...
		{
			temp_pathlist[PathPointCount] = tempIndex;
			PathPointCount++;
		...
		temp_pathlist[PathPointCount] = from;
		PathPointCount++;

		// Reverse them...
		for (i = 0; i < PathPointCount; i++)
		{
			pathlist[i] = temp_pathlist[PathPointCount-(i+1)];
		}
	...
	int ASTAR_ShortenPath(int old_pathlist_size, int *old_pathlist, int *pathlist)
	{
		...
		temp_pathlist[PathPointCount] = old_pathlist[old_pathlist_size-1];
		PathPointCount++;
...
#define MAX_NODELINKS 32
	...
	void AIMod_TimeMapPaths()
	{
		...
		G_Printf( "Finding bot objectives at node number %i (%f %f %f).\n",
		...
		for ( i = 0; i < MAX_GENTITIES; i++ )
		{
			...
			if (ORIGINAL_SIZE > 0)
				G_Printf( "Objective %i (%s) pathsize is %i (unshortened %i).\n", i, goal->classname, pathsize, ORIGINAL_SIZE );
			else
