...
static void PM_SetVehicleAngles( vec3_t normal )
{
	...
	if ( pVeh->m_pVehicleInfo->rollLimit > 0 )
	{
		...
		if ( speed > 32.0f || speed < -32.0f ) 
		{
			...
			speed *= sin( (150 + pml.frametime) * 0.003f );
...
static qboolean PM_CheckJump( void ) 
{
	...
	if ( BG_IsSprinting (pm->ps, &pm->cmd) || (pm->ps->sprintDebounceTime + 500) > pm->cmd.serverTime )
	{
	    return qfalse;
	}
...
static void PM_WalkMove( void ) {
	...
	*/
	...
	// Xy: sprinting!
	if ( BG_IsSprinting (pm->ps, &pm->cmd) && pm->ps->sprintDebounceTime <= pm->cmd.serverTime )
	{
...
static int PM_TryRoll( void )
{
	...
	/*
	if ( weaponData[pm->ps->weapon].weaponRoll ( pm->ps->weapon != WP_SABER && pm->ps->weapon != WP_MELEE) ||
		PM_IsRocketTrooper() ||
		BG_HasYsalamiri(pm->gametype, pm->ps) ||
		!BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime, FP_LEVITATION))
...
static void PM_Footsteps( void ) {
	...
	else
	{
		...
		else if ( BG_IsSprinting (pm->ps, &pm->cmd) )
		{
		    bobmove = 0.6f;
...
void PM_BeginWeaponChange( int weaponId ) {
    ...
    if ( weaponId > 0 && !BG_HasWeaponItem (pm->ps->clientNum, weaponId) )
	{
	    return;
	}
...
//---------------------------------------
{
	...
	else
	{
		...
		if ( pm->cmd.buttons & BUTTON_ATTACK )
		{
		    if ( thisWeaponData->zoomType != ZOOM_NONE )
		    {
		        if ( thisWeaponData->firemodes[0].chargeTime &&
		                pm->ps->stats[STAT_AMMO] >= thisWeaponData->firemodes[0].cost)
			    {
                    altFire = qfalse;
                    charging = qtrue;
		        }
			...
			else if ( !charging && thisWeaponData->firemodes[0].chargeTime && pm->ps->stats[STAT_AMMO] >= thisWeaponData->firemodes[0].cost )
			{
				charging = qtrue;
			}
		}
	...
	if ( charging )
	{
	    weaponFireModeStats_t *weaponFireData = &GetWeaponData( pm->ps->weapon, pm->ps->weaponVariation )->firemodes[altFire];
		...
		if ( altFire )
		{
			if ( pm->ps->weaponstate != WEAPON_CHARGING_ALT )
			{
				...
				BG_AddPredictableEventToPlayerstate(EV_WEAPON_CHARGE_ALT, (pm->ps->weapon << 8) | (pm->ps->weaponVariation & 0xFF), pm->ps);
			}
			...
			{
				// Jedi Knight Galaxies, handle clips properly
				if ( GetWeaponAmmoClip( pm->ps->weapon, pm->ps->weaponVariation ) )
				{
					if ( pm->ps->stats[STAT_AMMO] < ( weaponFireData->cost + weaponFireData->cost ))
					{
						// Jedi Knight Galaxies - If we're running low, see if we can still fully charge and fire the shot, even if it'll reduce our ammo to 0
						pm->ps->weaponstate = WEAPON_CHARGING_ALT;
...
static void PM_Weapon( void )
{
	...
	*/
	...
	{
		if ( pm->ps->clientNum < MAX_CLIENTS && pm->ps->ammo[ GetWeaponAmmoIndex( pm->ps->weapon, pm->ps->weaponVariation ) ] != -1 )
		{
			// enough energy to fire this weapon?
			//if (pm->ps->ammo[GetWeaponAmmoIndex( pm->ps->weapon, pm->ps->weaponVariation )] < GetWeaponData( pm->ps->weapon, pm->ps->weaponVariation )->primary.cost &&
			...
			if (weaponData[pm->ps->weapon].useClip) {
				if (pm->ps->clipammo[pm->ps->weapon] < GetWeaponData( pm->ps->weapon, pm->ps->weaponVariation )->primary.cost &&
					pm->ps->clipammo[pm->ps->weapon] < GetWeaponData( pm->ps->weapon, pm->ps->weaponVariation )->secondary.cost) 
			...
			if (pm->ps->weapon == WP_DET_PACK && !pm->ps->hasDetPackPlanted && pm->ps->ammo[GetWeaponAmmoIndex( pm->ps->weapon, pm->ps->weaponVariation )] < 1)
			{
				PM_AddEventWithParm( EV_NOAMMO, WP_NUM_WEAPONS+pm->ps->weapon );
	...
	*/
		...
		if ( PM_CanSetWeaponAnims())
		{
			...
			else
			{
				...
				else*/
				{
					...
					{
						PM_StartTorsoAnim( weaponData->torsoReadyAnimation );
					}
				}
			}
		}
	...
	if (PM_CanSetWeaponAnims())
	{
	    if ( !PM_IsRocketTrooper() &&
		    ...
		    pm->ps->torsoTimer <= 0 &&
		    (pm->ps->torsoAnim) != weaponData->torsoReadyAnimation &&
	...
	else
	{
	    if ( pm->ps->shotsRemaining & SHOTS_TOGGLEBIT )
	    {
	        if ( weaponData->firemodes[0].firingType == FT_SEMI )
	        {
	            return;
	        }
	        else if ( weaponData->firemodes[0].firingType == FT_BURST )
	        {
	            pm->ps->shotsRemaining = weaponData->firemodes[0].shotsPerBurst & ~SHOTS_TOGGLEBIT;
	        }
	    }
	}
	...
	if (pm->ps->weapon == WP_EMPLACED_GUN)
	{
		addTime = weaponData->firemodes[0].delay;
...
#endif
		/*
		addTime = weaponData[WP_EMPLACED_GUN].primary.delay;
		pm->ps->weaponTime += addTime;
		if ( (pm->cmd.buttons & BUTTON_ALT_ATTACK) )
		{
			PM_AddEvent( EV_ALT_FIRE );
	...
	if ( weaponData->zoomType != ZOOM_NONE && ( pm->cmd.buttons & BUTTON_ALT_ATTACK ) && !pm->ps->zoomLocked )
	{
		return;
	}

	/* Can't use zoom mode when you're using binoculars */
	if ( weaponData->zoomType != ZOOM_NONE && ( pm->cmd.buttons & BUTTON_ALT_ATTACK ) && pm->ps->zoomMode == 2 )
	{
		return;
	}
	...
	if (doStdAnim) {
		PM_StartTorsoAnim( weaponData->torsoFiringAnimation );
	}
	...
	else
	{
        ...
        switch ( weaponData->firemodes[0].firingType )
		{
		    case FT_AUTOMATIC:
		        addTime = weaponData->firemodes[0].delay;
		        break;
		    ...
		    case FT_BURST:
		        ...
		        {
		            addTime = weaponData->firemodes[0].burstFireDelay;
		            pm->ps->shotsRemaining = (pm->ps->shotsRemaining - 1) & ~SHOTS_TOGGLEBIT;
		        }
		        break;
		}
...
//-------------------------------------------
{
	...
	if (primFireDown)
	{
		amount = pm->ps->stats[STAT_AMMO] - weapon->firemodes[0].cost;
	}
	else
	{
	...
	if ( weapon->zoomType != ZOOM_NONE )
	{
	    if ( pm->ps->weaponstate == WEAPON_READY )
	    {
		            ...
		            (((pm->ps->ironsightsTime & ~IRONSIGHTS_MSB) + IRONSIGHTS_TIME + 50) <= pm->cmd.serverTime) )
		    {
			    ...
			    if ( !pm->ps->zoomMode && pm->ps->pm_type != PM_DEAD )
			    {
				    ...
				    pm->ps->zoomMode = 1;
				    pm->ps->zoomFov = weapon->startZoomFov;//cg_fov.value;
				    pm->ps->zoomLocked = qfalse;
				    pm->ps->zoomLockTime = pm->cmd.serverTime + 50;
				    
				    PM_AddEvent (EV_DISRUPTOR_ZOOMSOUND);
			    }
			    else if (pm->ps->zoomLockTime < pm->cmd.serverTime)
		        {
			        // Not pressing zoom any more
			        if ( pm->ps->zoomMode )
			        {
				        if (pm->ps->zoomMode == 1 && !pm->ps->zoomLocked && weapon->zoomType == ZOOM_CONTINUOUS)
				        { //approximate what level the client should be zoomed at based on how long zoom was held
					        pm->ps->zoomFov = ((pm->cmd.serverTime+50) - pm->ps->zoomLockTime) * (0.001f * weapon->zoomTime) / (weapon->startZoomFov - weapon->endZoomFov);
					        if (pm->ps->zoomFov > weapon->startZoomFov)
					        {
						        pm->ps->zoomFov = weapon->startZoomFov;
					        }
					        if (pm->ps->zoomFov < weapon->endZoomFov)
					        {
						        pm->ps->zoomFov = weapon->endZoomFov;
					        }
				        }
		    ...
		    (pm->cmd.rightmove && pm->ps->groundEntityNum != ENTITYNUM_NONE))
	    {
		    if ( pm->ps->zoomMode == 1 && pm->ps->zoomLockTime < pm->cmd.serverTime )
		    {
			    ...
			    // Xy: didn't really want to put this here, but oh well
			    pm->ps->weaponstate = WEAPON_READY;
	...
	// JKG: No firing while sprinting
    if ( BG_IsSprinting (pm->ps, &pm->cmd) )
    {
        pm->cmd.buttons &= ~(BUTTON_ATTACK | BUTTON_ALT_ATTACK);
    }
...
void BG_AdjustClientSpeed(playerState_t *ps, usercmd_t *cmd, int svTime)
{
	...
	/* When you're in zoom, you are focusing on the lens and not your surroundings. Alter the running speed */
	//if ( GetWeaponData( pm->ps->weapon, pm->ps->weaponVariation )->hasZoomMode && pm->ps->zoomMode == 1 && pm->ps->zoomLockTime < pm->cmd.serverTime )
	//{
	//	ps->speed *= 0.5f;
	//}
			...
			*/
			...
			for(j = 0; j < ARMSLOT_MAX; j++)
			{
				if(armorGhoul2 != NULL)
				{
					...
					{
						vec3_t tempAngles;
						VectorCopy(savedAngles, tempAngles);

						VectorScale(tempAngles, 0.6f, tempAngles);
						strap_G2API_SetBoneAngles(armorGhoul2[j], 0, "lower_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);

						VectorScale(tempAngles, 0.8f, tempAngles);
						strap_G2API_SetBoneAngles(armorGhoul2[j], 0, "upper_lumbar", tempAngles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);

						VectorScale(tempAngles, 0.8f, tempAngles);
						strap_G2API_SetBoneAngles(armorGhoul2[j], 0, "thoracic", tempAngles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
...
void PmoveSingle (pmove_t *pmove) {
	...
	if ( (pm->cmd.buttons & BUTTON_WALKING) && BG_IsSprinting (pm->ps, &pm->cmd) )
	{
	...
	if ( GetWeaponData( pm->ps->weapon, pm->ps->weaponVariation )->zoomType != ZOOM_NONE && pm->ps->zoomMode == 1 )
	{
		if ( (pm->cmd.buttons & BUTTON_ALT_ATTACK) && !( pm->cmd.buttons & BUTTON_ATTACK ) && pm->ps->zoomLocked )
		{
