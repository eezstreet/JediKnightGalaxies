...
//			RegisterItem( FindItemForWeapon( (weapon_t)(curWeap) ) );	//precache the weapon
			...
			ent->NPC->currentAmmo = ent->client->ps.ammo[GetWeaponAmmoIndex( curWeap, ent->client->ps.weaponVariation )] = 100;//FIXME: max ammo
...
//	} else
	...
	*/
	...
	{
		trap_G2API_InitGhoul2Model( &ent->client->weaponGhoul2[0], BG_FindItemForWeapon( ent->client->ps.weapon )->view_model, 0, 0, 0, 0, 0 );
	}
...
void SP_NPC_spawner( gentity_t *self)
{
	...
	if (temp) {
		G_NewString2(&self->npcscript, temp);
	}
...
gentity_t *NPC_SpawnType( gentity_t *ent, char *npc_type, char *targetname, qboolean isVehicle ) 
{
	...
	G_NewString2(&NPCspawner->NPC_type, npc_type);

	if ( targetname )
	{
		//NPCspawner->NPC_targetname = G_NewString(targetname);
		G_NewString2(&NPCspawner->NPC_targetname, targetname);
	}
