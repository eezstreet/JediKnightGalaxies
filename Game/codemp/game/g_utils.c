...
void G_ScaleNetHealth(gentity_t *self)
{
	...
	divfactor = ceil(log10(maxHealth)) - 3;
...
void G_UseDispenserOn(gentity_t *ent, int dispType, gentity_t *target)
{
	...
	else if (dispType == HI_AMMODISP)
	{
		...
		{ //do the next increment
			...
			target->client->ps.ammo[ammoIndex] += GetWeaponData( target->client->ps.weapon, target->client->ps.weaponVariation )->firemodes[0].cost;
