...
Format:
	...
	Flags must be restored to original after running this for everyone.
==================
*/
...
void TeamplayInfoMessage( gentity_t *ent )
{
	...
	if ( ent->client->pers.partyNumber == PARTY_SLOT_EMPTY && !ent->client->pers.teamInfo )
	{
		return;
	}
	...
	for ( i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if ( player == ent || !player->inuse || !player->client || !player->client->pers.partyUpdate )
		{
			continue;
		}

		if (((( player->client->sess.sessionTeam == TEAM_RED || player->client->sess.sessionTeam == TEAM_BLUE ) && player->client->sess.sessionTeam == ent->client->sess.sessionTeam )) || TeamFriendly( ent->s.number, player->s.number ))
		{
			...
			if ( !Q_stratt( buffer, sizeof( buffer ), va( "%i %i %i %i %i %i %i %i %i ", i, player->client->pers.teamState.location, ( h < 0 ) ? 0 : h, player->client->ps.stats[STAT_MAX_HEALTH], ( a < 0 ) ? 0 : a, player->client->ps.stats[STAT_MAX_ARMOR], player->client->ps.forcePower, mfp, player->client->ps.weapon )))
			{
...
void CheckTeamStatus( void )
{
	...
	if ( level.time - level.lastTeamLocationTime > 250 )
	{
		...
		for ( i = 0; i < MAX_CLIENTS; i++ )
		{
			...
			if ( ent->inuse && ( ent->client->pers.partyNumber != PARTY_SLOT_EMPTY || ( ent->client->sess.sessionTeam == TEAM_RED || ent->client->sess.sessionTeam == TEAM_BLUE )))
			{
				if (( loc = Team_GetLocation( ent )) != NULL )
				{
		...
		for ( i = 0; i < MAX_CLIENTS; i++ )
		{
			...
			if ( ent->client && ent->client->pers.connected == CON_CONNECTED && ent->client->pers.partyUpdate )
			{
				ent->client->pers.partyUpdate = qfalse;
			}
		}			
	}
}
