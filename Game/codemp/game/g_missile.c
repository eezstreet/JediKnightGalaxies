...
void G_MissileImpact( gentity_t *ent, trace_t *trace ) {
	...
	/* Other can take damage and is either a NPC or a client */
/*	if ( other->takedamage && ( other->client || other->NPC ) && ( ent->bounceCount > 0 || ent->bounceCount == -5 ) && ( ent->flags & ( FL_BOUNCE | FL_BOUNCE_HALF )))
	{
		//vec3_t newOrigin;
		//trace->plane.normal[2] = 0.4;
		//VectorCopy( trace->endpos, ent->r.currentOrigin );

		vec3_t newAngles, difference, backupDelta;

		VectorSubtract( trace->endpos, ent->r.currentOrigin, difference );
		vectoangles( difference, newAngles );
		newAngles[YAW] = -newAngles[YAW];

		G_DeflectMissile( other, ent, newAngles );
		//G_BounceMissile( ent, trace );
		G_AddEvent( ent, EV_GRENADE_BOUNCE, 0 );
		return;
	}*/
		...
		{//a hit with demp2 decloaks people, disables ships
			...
			else if ( other && other->client && other->client->ps.powerups[PW_CLOAKED] )
			{
				...
				{//direct hit with alt disables cloak forever
					...
					other->client->cloakToggleTime = Q3_TIMEINFINITE;
				}
				else
				{//temp disable
