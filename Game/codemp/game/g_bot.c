...
void G_CheckMinimumNpcs( void ) {
	...
	if (checkminimumnpcs_time > level.time - 1000) {
		return;
	}

	checkminimumnpcs_time = level.time;
	trap_Cvar_Update(&npc_minplayers);
	minplayers = npc_minplayers.integer;
	...
	if (botplayers < minplayers)
	{
		...
		G_Printf(va("Spawning %s at waypoint %i.\n", npc->NPC_type, waypoint));
...
#ifdef __NPC_MINPLAYERS__
	trap_Cvar_Register( &npc_minplayers, "npc_minplayers", "0", CVAR_SERVERINFO );
#endif //__NPC_MINPLAYERS__
