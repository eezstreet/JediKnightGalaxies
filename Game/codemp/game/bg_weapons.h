...
typedef struct weaponFireModeStats_s
{
	...
	char		disruptionWeapon;	// If true, this weapon mode uses a disruptor trace rather then a missile (instant).
	...
	short		delay;				// The delay between each shot/throw/whatever.
...
typedef struct
{
	...
	weaponFireModeStats_t primary;
    weaponFireModeStats_t secondary;
...
#define GetWeaponPrimaryCostSingle( a )		GetWeaponData( a, 0 )->primary.cost		// CG_WeaponCheck for weapon select drawing (Trash the function?)!
#define GetWeaponSecondaryCostSingle( a )	GetWeaponData( a, 0 )->secondary.cost	// CG_WeaponCheck for weapon select drawing (Trash the function?)!
