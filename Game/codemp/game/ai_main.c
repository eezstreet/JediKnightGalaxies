...
int ShouldSecondaryFire(bot_state_t *bs)
{
	...
	if ( bs->cur_ps.ammo[GetWeaponAmmoIndex( weap, bs->cur_ps.weaponVariation )] < GetWeaponData( weap,bs->cur_ps.weaponVariation )->secondary.cost )
	{
		return 0;
	}
...
{ //out of ammo, resort to the first weapon we come across that has ammo
	...
	{
		if (bs->cur_ps.ammo[GetWeaponAmmoIndex( bs->cur_ps.weapon, bs->cur_ps.weaponVariation )] >= GetWeaponData( bs->cur_ps.weapon, bs->cur_ps.weaponVariation )->primary.cost &&
