...
int ShouldSecondaryFire(bot_state_t *bs)
{
	...
	if ( bs->cur_ps.ammo[GetWeaponAmmoIndex( weap, bs->cur_ps.weaponVariation )] < GetWeaponData( weap,bs->cur_ps.weaponVariation )->firemodes[1].cost )
	{
		return 0;
	}
...
{ //out of ammo, resort to the first weapon we come across that has ammo
	...
	{
		if (bs->cur_ps.ammo[GetWeaponAmmoIndex( bs->cur_ps.weapon, bs->cur_ps.weaponVariation )] >= GetWeaponData( bs->cur_ps.weapon, bs->cur_ps.weaponVariation )->firemodes[0].cost &&
			(bs->cur_ps.stats[STAT_WEAPONS] & (1 << i)))
		{
		    int index = BG_GetWeaponIndexFromClass (i, 0);
			bs->virtualWeapon = i;
	...
	{ //should always have this.. shouldn't we?
	    int index = BG_GetWeaponIndexFromClass (1, 0);
		bs->virtualWeapon = 1;
		BotSelectWeapon(bs->client, index);
		//bs->cur_ps.weapon = 1;
...
int BotSelectIdealWeapon(bot_state_t *bs)
{
	...
	{
	    int index = BG_GetWeaponIndexFromClass (bestweapon, 0);
