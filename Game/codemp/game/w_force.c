	...
	if ( traceEnt && traceEnt->takedamage )
	{
		...
		{//an enemy or object
			...
			{ //give them power and don't hurt them.
				traceEnt->client->ns.forcePower++;
				if (traceEnt->client->ns.forcePower > traceEnt->client->ps.fd.forcePowerMax)
				{
					traceEnt->client->ns.forcePower = traceEnt->client->ps.fd.forcePowerMax;
				}
				return;
			}
...
static void WP_ForcePowerRun( gentity_t *self, forcePowers_t forcePower, usercmd_t *cmd )
{
	...
	case FP_LIGHTNING:
		...
		else
		{
			...
			BG_ForcePowerDrain( &self->client->ps, &self->client->ns, forcePower, 0 );
		}
		break;
	case FP_TELEPATHY:
		if ( self->client->holdingObjectiveItem >= MAX_CLIENTS  
