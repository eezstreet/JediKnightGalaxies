...
-- Try linking up to a fellow unlinked node
function ENT:TryLinking()
	local mins, maxs = self:GetBoundingBox()
	local base = self:GetPos()
	mins = mins * 2
	maxs = maxs * 2
	local list = ents.FindInBox(base + mins, base + maxs)
	for k,v in pairs(list) do
		if v == self then
			-- Ignore
		elseif v:GetClassName() == "pzk_tourneynode" then
			if not v.mpenabled then
				v:DoLinkup(self)
				self.mpenabled = true
				self.mplink = v
				break
...
-- Called by another ent when a linkup is requested
function ENT:DoLinkup(other)
	if self.mpenabled then
		return
	end
	self.mpenabled = true
	self.mplink = other
...
function ENT:OnUse(other, activator)
	...
	local dlg = dialogue.CreateDialogueObject("PzkTourneyNode")
	if not dlg then
		print("ERROR: Cannot find dialogue 'PzkTourneyNode'")
		return
	end
	ply:SetGodMode(true)
	ply:SetNoMove(true)
