--[[ ------------------------------------------------
	...
	Stargate controls - Tatooine GM Hideout
	
	Written by BobaFett
--------------------------------------------------]]

-- Initialize the GM Hideout on Tatooine
JKG.GMH.StarGate = {}
...
local function InitEnts()
	...
	-- TEMP
	if not SGEnts.Horizon then
		local entfact = ents.CreateEntityFactory("fx_runner")
		local org = SGEnts.Ring:GetPos()
		entfact:SetSpawnVar("origin", string.format("%i %i %i", org.x, org.y, org.z))
		entfact:SetSpawnVar("fxfile", "env/btend")
		entfact:SetSpawnVar("angles", "0 180 90")
		entfact:SetSpawnVar("targetname", "StargateHorizon")
		entfact:SetSpawnVar("delay", "50")
		entfact:SetSpawnVar("spawnflags", "1")
		SGEnts.Horizon = entfact:Create()
	end
end
...
local function RedirSpawnOverride(ply)
	...
	local cin = cinematics.Get("sg_tatooine")
	if cin then
