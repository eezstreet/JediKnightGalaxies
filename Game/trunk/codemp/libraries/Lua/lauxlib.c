...
LUALIB_API int luaL_typerror (lua_State *L, int narg, const char *tname) {
	...
	if (lua_type(L,narg) == LUA_TUSERDATA) {
		// Check for GLua Objects
		lua_getmetatable(L,narg);
		lua_getfield(L,-1,"ObjName");
		if (!lua_isnil(L,-1)) {
			// Its a GLua Object, show the proper name, rather than 'userdata'
			msg = lua_pushfstring(L, "%s expected, got %s",	tname, lua_tostring(L,-1));
			lua_replace(L, -3); // Put it in 3rd place, so we can pop the metatable and field
			lua_pop(L, 2);
		} else {
			lua_pop(L,2);
