...
//#undef INTEL_COMPILER // UQ1: Who the f*ck added this?
...
#ifdef WIN32
#define UQ_INLINE __forceinline
#else
#define UQ_INLINE inline
#endif
...
/*UQ_INLINE static void swap16(uint16* p) { *p = ((*p >> 8) & 0xff) | (*p << 8); }
UQ_INLINE static void swap32(uint32* p) { *p = ((*p >> 24 & 0xff)) | ((*p >> 8) & 0xff00) | ((*p << 8) & 0xff0000) | (*p << 24); }
UQ_INLINE static void swap64(uint64* p) { *p = ((*p >> 56)) | ((*p >> 40) & 0x000000000000ff00ULL) | ((*p >> 24) & 0x0000000000ff0000ULL) | ((*p >> 8 ) & 0x00000000ff000000ULL) |
								((*p << 8 ) & 0x000000ff00000000ULL) | ((*p << 24) & 0x0000ff0000000000ULL) | ((*p << 40) & 0x00ff000000000000ULL) | ((*p << 56)); }*/

UQ_INLINE static void swap16(uint16* p) { *p = bswap_16((uint16_t)*p); }
UQ_INLINE static void swap32(uint32* p) { *p = bswap_32((uint32_t)*p); }
UQ_INLINE static void swap64(uint64* p) { *p = bswap_64((uint64_t)*p);; }

UQ_INLINE static float swapfloat(float p)
{
...
UQ_INLINE static double swapdouble(double p)
{
...
UQ_INLINE static void swapfloat(float * p)
{
	union { float asfloat; uint8 asbytes[4]; } u1, u2;
	u1.asfloat = *p;
	/* swap! */
...
UQ_INLINE static void swapdouble(double * p)
{
	union { double asfloat; uint8 asbytes[8]; } u1, u2;
	u1.asfloat = *p;
	/* swap! */
...
/*UQ_INLINE static uint16 swap16(uint16 p) { return ((p >> 8) & 0xff) | (p << 8); }
UQ_INLINE static uint32 swap32(uint32 p) { return ((p >> 24) & 0xff) | ((p >> 8) & 0xff00) | ((p << 8) & 0xff0000) | (p << 24); }
UQ_INLINE static uint64 swap64(uint64 p)  { p = (((p >> 56) & 0xff)) | ((p >> 40) & 0x000000000000ff00ULL) | ((p >> 24) & 0x0000000000ff0000ULL) | ((p >> 8 ) & 0x00000000ff000000ULL) |
								((p << 8 ) & 0x000000ff00000000ULL) | ((p << 24) & 0x0000ff0000000000ULL) | ((p << 40) & 0x00ff000000000000ULL) | ((p << 56)); }

UQ_INLINE static void swap16(int16* p) { *p = ((*p >> 8) & 0xff) | (*p << 8); }
UQ_INLINE static void swap32(int32* p) { *p = ((*p >> 24) & 0xff) | ((*p >> 8) & 0xff00) | ((*p << 8) & 0xff0000) | (*p << 24); }
UQ_INLINE static void swap64(int64* p) { *p = ((*p >> 56) & 0xff) | ((*p >> 40) & 0x000000000000ff00ULL) | ((*p >> 24) & 0x0000000000ff0000ULL) | ((*p >> 8 ) & 0x00000000ff000000ULL) |
								((*p << 8 ) & 0x000000ff00000000ULL) | ((*p << 24) & 0x0000ff0000000000ULL) | ((*p << 40) & 0x00ff000000000000ULL) | ((*p << 56)); }

UQ_INLINE static int16 swap16(int16 p) { return ((p >> 8) & 0xff) | (p << 8); }
UQ_INLINE static int32 swap32(int32 p) { return ((p >> 24) & 0xff) | ((p >> 8) & 0xff00) | ((p << 8) & 0xff0000) | (p << 24); }
UQ_INLINE static int64 swap64(int64 p)  { return ((((p >> 56) & 0xff)) | ((p >> 40) & 0x000000000000ff00ULL) | ((p >> 24) & 0x0000000000ff0000ULL) | ((p >> 8 ) & 0x00000000ff000000ULL) |
								((p << 8 ) & 0x000000ff00000000ULL) | ((p << 24) & 0x0000ff0000000000ULL) | ((p << 40) & 0x00ff000000000000ULL) | ((p << 56))); }*/

UQ_INLINE static uint16 swap16(uint16 p) { return bswap_16((uint16_t)p); }
UQ_INLINE static uint32 swap32(uint32 p) { return bswap_32((uint32_t)p); }
UQ_INLINE static uint64 swap64(uint64 p)  { return bswap_64((uint64_t)p); }

UQ_INLINE static void swap16(int16* p) { *p = bswap_16((uint16_t)*p); }
UQ_INLINE static void swap32(int32* p) { *p = bswap_32((uint32_t)*p); }
UQ_INLINE static void swap64(int64* p) { *p = bswap_64((uint64_t)*p); }

UQ_INLINE static int16 swap16(int16 p) { return bswap_16((uint16_t)p); }
UQ_INLINE static int32 swap32(int32 p) { return bswap_32((uint32_t)p); }
UQ_INLINE static int64 swap64(int64 p)  { return bswap_64((uint64_t)p); }
...
UQ_INLINE void reverse_array(uint8 * pointer, size_t count)
{
	size_t x;
...
UQ_INLINE void UQ_TOLOWER(std::string& str)
{
	for(size_t i = 0; i < str.length(); ++i)
		str[i] = (char)tolower(str[i]);
};

UQ_INLINE void UQ_TOUPPER(std::string& str)
{
	for(size_t i = 0; i < str.length(); ++i)
