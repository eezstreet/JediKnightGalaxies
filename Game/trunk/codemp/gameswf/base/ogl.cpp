...
namespace ogl {
	...
	// Some extension checking code snipped from glut.
	{
		...
		whereis = (char *)strchr(extension, ' ');
		if (whereis || *extension == '\0') return false;
		...
		while (!supported)
		{
			...
			whereis = (char *)strstr((const char*)start, extension);
			if (!whereis) break;
			...
			supported &= (whereis == start) || (whereis[-1] == ' ');

			// Check for space after extension
			terminator = whereis + strlen(extension);
